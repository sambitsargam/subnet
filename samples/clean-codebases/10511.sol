...
contract ApyOracle {
  ...
  function getUniPrice(IUniswapV2Pair unipair) public view returns (uint256) {
    ...
    uint256 singlePriceInWeth = total / unipair.totalSupply();
    address[] memory p = new address[](2);
    p[0] = weth;
    p[1] = usdc;
    uint256[] memory prices = IUniswapRouterV2(oracle).getAmountsOut(singlePriceInWeth, p);
    return prices[1]; // price of single token in USDC
  }
}