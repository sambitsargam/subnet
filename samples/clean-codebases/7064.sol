...
// File contracts/nodes/DexUtils.sol
...
contract DexUtils {
  ...
  AggregatorV3Interface internal nativeUsdFeed;
  ...
  constructor(
    ...
    nativeUsdFeed = AggregatorV3Interface(_linkPriceFeed);
  ...
  /**
   * Returns the latest USD/NATIVE price with returned value at 18 decimals
   * https://docs.chain.link/docs/get-the-latest-price/
   */
  function getLatestNativePrice() public view returns (uint256) {
    uint8 decimals = nativeUsdFeed.decimals();
    (, int256 price, , , ) = nativeUsdFeed.latestRoundData();
    ...
    returns (uint256)
  {
    ...
    // 2022-02-11 LW: Use Chainlink price feed for native/usd price feed
    // instead of AMM to prevent flash loan exploits
    // (
    //   uint256 nsPriceAdjusted,
    //   address nsToken0,
    //   address nsToken1
    // ) = _getTokenPrice(wrappedNative, stableToken);
    uint256 nsPriceAdjusted = getLatestNativePrice();
    address nsToken0 = wrappedNative;
    address nsToken1 = stableToken;
