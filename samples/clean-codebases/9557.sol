...
// File @openzeppelin/contracts/token/ERC721/extensions/ERC721Pausable.sol@v4.5.0
...
contract Bansho is ERC721Enumerable ,ERC721Burnable , ERC2981, ERC721Pausable, Ownable, AccessControlEnumerable {
    ...
    function tokenURI(uint256 tokenId_) public view override returns (string memory) {
        ...
        bytes memory json = abi.encodePacked(_metadata[tokenId_]);
        return string(abi.encodePacked("data:application/json;base64,", Base64.encode(json)));
    }

    function bulkmint(address[] memory to_, uint256[] memory tokenId_) public {
        require(hasRole(MINTER_ROLE, _msgSender()), "must have minter role");
        require(to_.length == tokenId_.length, "length must be same");
        for(uint256 i; i < to_.length; i++){
            _safeMint(to_[i], tokenId_[i]);
        }
    }
    ...
    function dataStorage() public view onlyOwner returns (DataStorage[] memory){
        return _dataStorage;
    }

    function setMetadataBulk(uint256[] memory tokenId_, string[] memory metadata_) public {
        require(hasRole(MINTER_ROLE, _msgSender()), "must have minter role");
        require(tokenId_.length == metadata_.length, "length must be same");
        for(uint256 i; i < tokenId_.length; i++){
             _metadata[tokenId_[i]] = metadata_[i];
        }
    }
