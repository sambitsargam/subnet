// TG: https://t.me/rushapeeth  , Tax 13% BOTH
...
contract RUSHAPEETH is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "RUSH APE ETH";
    string private _symbol = "RU$APE";
    uint8 private _decimals = 9;
    uint256 private _Supply = 100 * 10**9 * 10**9;

    uint256 public _liquidityFee = 2;
    uint256 public _marketingFee = 7;
    uint256 public _devFee = 4;   
    uint256 public _antidump = 0;  
    uint256 public _giveaway = 0;
    uint256 public _setbot = 0;
     uint256 public _maxtransaction = 3 * 10**9 * 10**9;
    uint256 public _walletMax = 4 * 10**9 * 10**9;
    ...
    address payable public marketingWalletAddress = payable(0x1391379a03800EA71752D5a8AFedf842b2aeC454); 
    address payable public devWalletAddress = payable(0xE5636b52cA9F35370768c5499313A97695D03F59); 
    ...
    constructor () {
        ...
        _giveaway = _liquidityFee.add(_marketingFee).add(_devFee);
        _setbot = _giveaway.add(_antidump);
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return _Supply.sub(balanceOf(deadAddress));
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 tokensForLP = tAmount.div(_giveaway).mul(_liquidityFee).div(2);
        uint256 tokensForSwap = tAmount.sub(tokensForLP);

        swapTokensForEth(tokensForSwap);
        uint256 amountReceived = address(this).balance;

        uint256 totalFee = _giveaway.sub(_liquidityFee.div(2));
        
        uint256 amountLiquidity = amountReceived.mul(_liquidityFee).div(totalFee).div(2);
        uint256 amountRewards = amountReceived.mul(_devFee).div(totalFee);
        ...
        transferToAddressETH(devWalletAddress, amountRewards);
