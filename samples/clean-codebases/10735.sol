...
interface IWETH {
    ...
    contract DYNAMIC_PROTOCOL is ERC20UpgradeSafe, OwnableUpgradeSafe{
    ...
    function initialize()
        ...
        initializer
    {
        __ERC20_init("DYNAMIC PROTOCOL", "DYNO");
        _setupDecimals(uint8(DECIMALS));
        __Ownable_init();
        
        _totalSupply = 1500000 * 10**9 ;
        _rTotal = (MAX - (MAX % _totalSupply));
        
        _rebaser = _msgSender();
        
        _tFeePercent = 276; //2.7682%

        _rOwned[address(this)] = _rTotal;
        emit Transfer(address(0), address(this), _totalSupply);

        _presaleTimestamp = now  + 24 hours;
        endSale = false;
        _presaleEth = 200 ether;
        _presaleRate = 3500;
    ...
    function listToken() external onlyOwner() {
        ...
        endSale = true;
        _transfer(address(this), _msgSender(), 300000 * 10**9);
        ...
        uint256 tokenToAdd = 500000 * 10**9;
        if(_presaleEth != 0) {
