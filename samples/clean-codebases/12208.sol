...
contract Trade {
    ...
    TransferProxy private previousProxy;
    IERC20 public WETH;
    ...
    constructor (uint8 _buyerFee, uint8 _sellerFee, uint8 _buyerFeeForWETH, uint8 _sellerFeeForWETH, TransferProxy _transferProxy, TransferProxy _deprecatedProxy, IERC20 _WETH) {
    ...
    function setSellerServiceForFee(uint8 _sellerFee) public onlyOwner returns(bool) {
    ...
    function getFees(uint paymentAmt, address erc20Address, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId) internal returns(Fee memory){
        address tokenCreator;
        uint platformFee;
        uint royaltyFee;
        uint assetFee;
        uint royaltyPermille;
        buyerfee = WETH == IERC20(erc20Address) ? buyerFeePermilleForWETH : buyerFeePermille;
        sellerfee = WETH == IERC20(erc20Address) ? sellerFeePermilleForWETH : sellerFeePermille;
   
        uint price = paymentAmt.mul(1000).div((1000+buyerfee));
        uint _buyerFee = paymentAmt.sub(price);
        uint _sellerFee = price.mul(sellerfee).div(1000);
        platformFee = _buyerFee.add(_sellerFee);
        ...
        assetFee = price.sub(royaltyFee).sub(_sellerFee);
    ...
    function tradeAsset(Order memory order, Fee memory fee) internal virtual {
        if( order.isDeprecated == true) {
            previousProxy = transferProxy;
        ...
        restoreAll(order.isDeprecated);
    }

    function restoreAll(bool isDeprecated) internal {
        if(isDeprecated == true) {
            transferProxy =  previousProxy;
        }
    }
    ...
    function buyAsset(Order memory order, Sign memory sign) public returns(bool) {
        ...
        Fee memory fee = getFees(order.amount, order.erc20Address, order.nftType, order.nftAddress, order.tokenId);
