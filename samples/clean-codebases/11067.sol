...
// hevm: flattened sources of src/FTC.sol
// SPDX-License-Identifier: MIT
pragma solidity =0.8.12 >=0.8.0 <0.9.0;
...
Tax for Buying/Selling: 10%
    ...
    - 6% of each transaction sent to Charity Wallet
...
contract FTC is Ownable, IERC20 {
    ...
    constructor() {
        charityWallet = 0x9EE764513a0b450e6b23986597CeF30e0cDDcFED;
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "FTC: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "FTC: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "FTC: Sender is blacklisted");
        require(!isBlacklisted[recipient], "FTC: Recipient is blacklisted");

        require(sender != address(0), "FTC: transfer from the zero address");
        require(recipient != address(0), "FTC: transfer to the zero address");
        ...
        require(
            ...
            "FTC: transfer amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "FTC: approve from the zero address");
        require(spender != address(0), "FTC: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "FTC: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    }
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "FTC: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "FTC: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }
    ...
    function _executeSwap(uint256 tokens, uint256 native) private {
        ...
        if (address(charityWallet) != address(0)) {
            swapTokensMarketing = (tokens * charityFeeBPS) / totalFeeBPS;
        }
        ...
        if (nativeMarketing > 0) {
            payable(charityWallet).transfer(nativeMarketing);
        }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "FTC: DEX pair can not be removed");
    ...
    function compound() public {
        require(compoundingEnabled, "FTC: no compounding");
    ...
    function setMaxTxBPS(uint256 bps) external onlyOwner {
        require(bps >= 50 && bps <= 10000, "BPS oor");
        maxTxBPS = bps;
    }
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "FTC_DividendTracker";
    string private _symbol = "FTC_DividendTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "FTC_DT: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "FTC_DT: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "FTC_DT: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "FTC_DT: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
