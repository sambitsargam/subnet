...
contract Escrow is Ownable {
    ...
    event AgreementDefaulted(uint256 initialDebt, uint256 accumulatedDebt);
    ...
    constructor() {
        ...
        tokens.initialEYEDeposit = EYE(607521);
        tokens.initialUSDCDebt = USDC(543735);
    ...
    function initializeAgreement(uint256 USDC_units) public {
        require(USDC_units < 543736, "ESCROW: Check your input");
    ...
    function payDebtFrom(uint256 USDC_units, address payer) internal {
        ...
        if (agreementState.accumulatedRepayments >= tokens.initialUSDCDebt) {
            ...
            uint256 diff =
                agreementState.accumulatedRepayments - tokens.initialUSDCDebt;

            usdc = diff < usdc ? usdc - diff : 0;
            agreementState.accumulatedRepayments = tokens.initialUSDCDebt;
        }
