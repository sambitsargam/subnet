...
contract TYRProtocol is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "TYR Protocol";
    string private _symbol = "TYR";
    ...
    uint256 internal _tokenTotal = 50_000e18;
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "TYR: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'TYR: The contract it self cannot be excluded');
        require(!_isExcluded[account], "TYR: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "TYR: Account is already included");
