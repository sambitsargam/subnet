...
contract Loot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function pluck(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray) internal view virtual returns (string memory) {
...
contract MutatingLoot is Loot {
    ...
    function previousOwnerOf(uint256 tokenId) public view virtual returns (address) {
        address owner = _previousOwners[tokenId];
        return owner;
    }

    function claim(uint256 tokenId) public payable nonReentrant {
        require(tokenId > 0 && tokenId < 7778, "Token ID invalid");
        require(msg.value >= minPrice, "not enough ETH was sent, need to be at least the minPrice");
        _safeMint(_msgSender(), tokenId);
    }
    
    function ownerClaim(uint256 tokenId) public nonReentrant onlyOwner {
        require(tokenId > 7777 && tokenId < 8077, "Token ID invalid");
        _safeMint(owner(), tokenId);
    }
    ...
    function pluck(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray) override internal view virtual returns (string memory) {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, previousOwnerOf(tokenId), ownerOf(tokenId), toString(tokenId))));
        string memory output = sourceArray[rand % sourceArray.length];
        uint256 greatness = rand % 21;
        if (greatness > 14) {
            output = string(abi.encodePacked(output, " ", suffixes[rand % suffixes.length]));
    ...
    function _beforeTokenTransfer(
        ...
        super._beforeTokenTransfer(from, to, tokenId);
        _previousOwners[tokenId] = from;
    }
    
    constructor() Loot("MuLoot", "MULOOT") {
       minPrice = 4e16; // initially set to 0.04 ETH
    }
