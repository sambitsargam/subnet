...
interface IPiPriceFeed {
    function wETH() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract PiMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function createOffer(
        ...
        IPiAuction auction = IPiAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isPiNFT(_nftAddress), "invalid nft address");
    ...
    function _isPiNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.pip() == _nftAddress ||
            IPiNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IPiNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IPiNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IPiNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IPiPriceFeed priceFeed = IPiPriceFeed(
