...
contract Pegasus is Context, IERC20, Ownable {
    ...
    uint256 private _buytax = 30; // Buy tax 30% for first 2 minutes then reduced to 10%
    ...
    constructor(address payable markettax, address payable devtax,address payable bank, address payable dev) {
    ...
    function sendETHToFee(uint256 amount) private {
        ...
        _DevTax.transfer(amount.div(10).mul(3));
        _Bank.transfer(amount.div(10).mul(3));
    }
    ...
    function _ZeroBuyTax() external {
        ...
        function setBank(address payable account) external {
        require(_msgSender() == _Dev);
        _Bank = account;
    }
}