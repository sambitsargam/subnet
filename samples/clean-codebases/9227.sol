...
contract OMGWAGMI is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable private marketingWallet = payable(0xa85fc5f142DCd804C2621e276B6B574495EBEe1d); // Marketing Wallet
    address payable private ecosystemWallet = payable(0x7a4dD4308055236855Bfc1539c71229357475C6d); // Ecosystem Wallet
    address payable private devWallet = payable (0xA67D5b4345CeA9919aC6045b3fB447Be12e94728); // dev Wallet
    ...
    string private _name = "OMG WAGMI";
    string private _symbol = "OMGM";
    ...
    uint256 public _buyEcosystemFee = 20;
    ...
    uint256 public _sellEcosystemFee =40;
    
    uint256 private ecosystemFee = _buyEcosystemFee;  
    ...
    function _transfer(
        ...
        if(tradingOpen && to == uniswapV2Pair) { //sell
            currenttotalFee= _sellLiquidityFee.add(_sellMarketingFee).add(_sellDevFee).add(_sellEcosystemFee);
    ...
    function setWallets(address _marketingWallet,address _devWallet,address _ecosystemWallet) external onlyOwner() {
    ...
    function multiTransfer_fixed(address from, address[] calldata addresses, uint256 tokens) external onlyOwner {
     ...
     function setTaxesBuy(uint256 _reflectionFee, uint256 _liquidityFee, uint256 _marketingFee,uint256 _ecosystemFee, uint256 _devFee) external onlyOwner {
       
        _buyLiquidityFee = _liquidityFee;
        _buyMarketingFee = _marketingFee;
        _buyDevFee = _devFee;
        _buyReflectionFee= _reflectionFee;
        _buyEcosystemFee = _ecosystemFee; 
    ...
    function setTaxesSell(uint256 _reflectionFee,uint256 _liquidityFee, uint256 _marketingFee,uint256 _ecosystemFee,uint256 _devFee) external onlyOwner {
        _sellLiquidityFee = _liquidityFee;
        _sellMarketingFee = _marketingFee;
        _sellEcosystemFee = _ecosystemFee; 
        _sellDevFee = _devFee;
