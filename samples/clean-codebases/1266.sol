...
// File: contracts/InulandStaking.sol

//SPDX-License-Identifier: MIT
pragma solidity 0.8.7;

/**
 * @title Inuland Staking Contract
 * @author Inuland Dev
 */
...
contract InulandStaking is ERC20("INUS", "INUS"), Ownable {
    ...
    /**
     *    @notice Keeping track of user and its token
     */
    struct UserInfo {
    ...
    /**
     *    @notice map User Addresses to their info
     */
    mapping(address => UserInfo) public stakeUserInfo;

    /**
     *    @notice staked nft => user address
     */
    ...
    //extending functions for Furture upgrades
    ...
    function setMiningPowerData(uint256[] memory _miningPowerData)
        public
        onlyOwner
    {
        miningPower = _miningPowerData;
    }
    ...
    function stake(uint256 _id, uint256 _lockedDays) public {
        _stake(msg.sender, _id, _lockedDays);
    }

    function batchStake(uint256[] memory _ids, uint256 _lockedDays) public {
        for (uint256 i = 0; i < _ids.length; ++i) {
            _stake(msg.sender, _ids[i], _lockedDays);
        }
    }
    ...
    function getMiningPower(uint256 _token) public view returns (uint256) {
        if (_token != 0) {
            return miningPower[_token - 1];
        } else {
            return 0;
        }
    }
    ...
    function getStakingRewards(address _user) public view returns (uint256) {
        ...
        for (uint256 i; i < user.stakedTokens.length; i++) {
            if (user.stakedTokens[i] != 0) {
                uint256 nftMiningPower = getMiningPower(user.stakedTokens[i]);
                //first time
    ...
    function _unstake(address _user, uint256 _id) internal {
        ...
        user.stakedTokens.removeElement(_id);
        user.amountStaked -= 1;
