/*  Telegram: https://t.me/starburstship
    website: https://starburstship.io
*/

/*  Name: Starburst
    Total Tokens: 100 Million
    Max Tx at launch: 2 million
    Max Wallet: 3%
    Buy tax: Never to exceed 10%
        10% at launch
        3% Future Development & Maintaining TAR
        3% Auto liquidity
        3% Marketing
        1% UltramanContracts
    Sell tax: Never exceed 15%
        10% at launch
        3% Future Development & Maintaining TBH
        3% Auto liquidity
        3% Marketing
        1% UltramanContracts
    ...
    This contract has been crafted by Ultraman Contracts for a client.
    Ultraman Contracts is not responsible if liquidity does not get locked. DYOR
    Ultraman contracts is not liable for the actions of the dev,
    but can attest to the fact the contract is safu as the following is true:
    ...
    ll info pertaining to the contract will be listed above this message.
...
contract Starburst is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 1000 * 1e5 * 1e9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "Starburst";
    string private constant _symbol = "TARB";
    ...
    mapping(address => bool) public boughtEarly; // mapping to track addresses that buy within the first 2 blocks pay a 3x tax for 48 hours to sell
    ...
    uint256 public maxTransactionAmount = (_tTotal * maxTxPercent) / 1000; // 2% maxTransactionAmountTxn
    ...
    constructor() payable {
        _rOwned[_msgSender()] = _rTotal / 1000 * 80;
        _rOwned[address(this)] = _rTotal / 1000 * 920;
        
        
        minimumTokensBeforeSwap = _tTotal * 5 / 10000; // 0.05% swap tokens amount
        
        marketingAddress = payable(0x81786f1bbDC155b464719Fd99DA515D3A7562fC8); // Marketing Address
        
        devAddress = payable(0xDf7fa4B4c3BCfDe90DF0849235a52ed80dfe6415); // Ultraman Address
        
        liquidityAddress = payable(0x9BCd8C3b26dd4AdB278cda6dbeFC3804840A9e3F); // Liquidity Address (switches to dead address once launch happens)
        ...
        emit Transfer(address(0), _msgSender(), _tTotal * 80 / 1000);
        emit Transfer(address(0), address(this), _tTotal * 920 / 1000);
    }
    ...
    function enableTrading() internal onlyOwner {
        ...
        earlyBuyPenaltyEnd = block.timestamp + 48 hours;
    }
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 ethForDev= ethForMarketing * 1 / 7;
