...
contract Mandelbulbs is ERC721Enumerable, Ownable , ReentrancyGuard {

  using Counters for Counters.Counter;
  Counters.Counter private _tokenIds;
  Counters.Counter private _founderTokenIds;
  ...
  uint256 private constant publicMintPrice = 0.05 ether;
  uint256 private constant maxNftPerAdress = 10;

  uint256 private constant presaleMintPrice = 0.04 ether;
  ...
  function setNotRevealedURI(string memory _notRevealedURI) public onlyOwner {
    ...
    function WhitelistedMint(uint256 _mintAmount , bytes32[] calldata _MerkleProof) external payable nonReentrant {
        ...
        require(preSaleActive == true , 'The PreSale is not currently open');
        require(_mintAmount != 0 && _mintAmount <= maxNftPerAdressPresale , 'Invalid Mint Amount');
        require(_tokenIds.current() + _mintAmount <= maxPublicBulbs , 'Not Enough Mandelbulbs Left');
        require(_mintAmount * presaleMintPrice == msg.value, 'Incorrect Amount of Ether Sent');
        
        uint256 userMintedAmount = addressMintedAmounts[msg.sender] + _mintAmount;
        require(userMintedAmount <= maxNftPerAdressPresale , 'Maximum Pre-Sale Limit Reached');
        ...
        addressMintedAmounts[msg.sender] = userMintedAmount;
    }

    function PublicMint(uint256 _mintAmount) external payable nonReentrant {
        require(!paused , 'The Contract is not Currently Open for Minting');
        require(publicSaleActive == true , 'Public Minting is Current Closed');
        require(_mintAmount != 0 && _mintAmount <= maxNftPerAdress , 'Invalid Mint Amount');
        require(_tokenIds.current() + _mintAmount <= maxPublicBulbs , 'Not Enough Mandelbulbs Left');
        require(_mintAmount * getCost() == msg.value, 'Incorrect Amount of Ether Sent');
        
        uint256 userMintedAmount = addressMintedAmounts[msg.sender] + _mintAmount;
        require(userMintedAmount <= maxNftPerAdress , 'Maximum NFT Limit Reached');
    ...
    function FounderMint(uint256 _mintAmount) external payable nonReentrant onlyOwner {
        require(_founderTokenIds.current() + _mintAmount <=  founderReserves , 'No NFTS Left' );
        require(_mintAmount != 0 && _mintAmount <= founderReserves , 'Invalid Mint Amount');

        uint256 userMintedAmount = _mintAmount + addressMintedAmounts[msg.sender];
        for (uint256 i = 0; i < _mintAmount; i++) {
            _founderTokenIds.increment();
            mint(_founderTokenIds.current() + maxPublicBulbs);
        }
        addressMintedAmounts[msg.sender] = userMintedAmount; 
    }
    ...
    function getCost() internal returns (uint256) {
        currentBlock = block.number;
        uint256 blocksFromStart = currentBlock - startingBlock;
        uint256 priceIncrement = blocksFromStart/35000;
        uint256 priceIncrease = (0.01 ether*priceIncrement);
        uint256 currentCost = publicMintPrice + priceIncrease;
        return currentCost;
    }
