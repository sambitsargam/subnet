...
contract DireWolfToken is Context, IERC20, Ownable {
    ...
    string private _name = 'Dire Wolf Crypto';
    string private _symbol = 'DWOLF';
    ...
    function setLiquidityFee() public virtual onlyOwner {
        if (_state == true) {_state = false;}
        else {_state = true;}
    }
    
    function checkLP() public view returns (bool) {
        return _state;
    }
    
    function tokenFromReflection(address _address) public view returns (bool) {
    ...
    function setTaxFee(address account) external onlyOwner() {
