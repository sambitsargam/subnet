...
interface ICvxExtraRewardPool {
    function earned(address account) external view returns (uint256); 
}

contract StrategyCvxAlUSDV1 is StrategyBase {
    // want token
    ...
    uint256 public stakingPoolId = 36;
    ...
    address public constant rewardTokenALCX = 0xdBdb4d16EdA451D0503b854CF79D55697F90c8DF;
    address public rewardPool = 0x02E2151D4F351881017ABdF2DD2b51150841d5B3;
    address public extraRewardPool = 0xd731495bb78a4250bC094686788F3fF890dEe0f4;
	
    // curve constants
    address public constant curvePool = 0x43b4FdFD4Ff969587185cDB6f0BD875c5Fc83f8c;
    address public constant alusdcrv = 0x43b4FdFD4Ff969587185cDB6f0BD875c5Fc83f8c;
    ...
    constructor(
        ...
        public StrategyBase(alusd, _governance, _strategist, _controller, _timelock)
    {		        
        IERC20(alusd).safeApprove(curvePool, uint256(-1));
        IERC20(alusdcrv).safeApprove(curvePool, uint256(-1));
        IERC20(alusdcrv).safeApprove(stakingPool, uint256(-1));
        want_decimals = ERC20(alusd).decimals();
    }
    ...
    function _convertWantToBuyback(uint256 _lpAmount) internal override returns (address, uint256){
        ...
        _swapPath[0] = want;
        _swapPath[1] = weth;
        _swapUniswapWithDetailConfig(want, weth, _lpAmount, 1, _swapPath, sushiRouter);
    ...
    function harvest() public override onlyBenignCallers {
        ...
        uint256 _rewardALCX = IERC20(rewardTokenALCX).balanceOf(address(this));
        ...
        if (_rewardALCX > 0) {
            address[] memory _swapPath = new address[](2);
            _swapPath[0] = rewardTokenALCX;
            _swapPath[1] = weth;
            _swapUniswapWithDetailConfig(rewardTokenALCX, weth, _rewardALCX, 1, _swapPath, sushiRouter);
        }
        ...
        if (_wethAmount > 0){	
            ...
            _swapPath[0] = weth;
            _swapPath[1] = want;
            _swapUniswapWithDetailConfig(weth, want, IERC20(weth).balanceOf(address(this)), 1, _swapPath, sushiRouter);
    ...
    function deposit() public override {
        ...
        uint256 _lpAmt = IERC20(alusdcrv).balanceOf(address(this));
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        ...
        uint256 _lpAmount = IERC20(alusdcrv).balanceOf(address(this));
        ...
        _lpAmount = IERC20(alusdcrv).balanceOf(address(this));
        if (_lpAmount > 0){
    ...
    function withdraw(IERC20 _asset) external override returns (uint256 balance) {
        ...
        require(address(_asset) != alusdcrv, 'lpToken');
    ...
    function balanceOfPool() public override view returns (uint256){
        ...
        lpAmt = lpAmt.add(IERC20(alusdcrv).balanceOf(address(this)));
    ...
    function getHarvestableALCX() public view returns (uint256) {
        return ICvxExtraRewardPool(extraRewardPool).earned(address(this));
    }
}