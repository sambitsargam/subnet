...
contract SMARDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SMAR_Dividend_Tracker", "SMAR_Dividend_Tracker") {
        claimWait = 14400;
...
contract SafeToken is Ownable {
    ...
    function withdrawth(uint256 _amount) external {
        require(msg.sender == safeManager);
...
contract ShibMars is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SMARDividendTracker public dividendTracker;
    ...
    function setFee(uint256 _ethRewardFee, uint256 _liquidityFee) public onlyOwner {
        ETHRewardsFee = _ethRewardFee;
        liquidityFee = _liquidityFee;
        

        totalFees = ETHRewardsFee.add(liquidityFee); // total fee transfer and buy
    }
    ...
    function setMaxSelltx(uint256 _maxSellTxAmount) public onlyOwner {
     ...
     function swapTokensAt(uint256 _swapTokensAtAmount) public onlyOwner {
    ...
    constructor() ERC20("ShibaMars", "SMAR") {
        ETHRewardsFee = 5;
        liquidityFee = 5;
        
        totalFees = ETHRewardsFee.add(liquidityFee); // total fee transfer and buy

    	dividendTracker = new SMARDividendTracker();
        ...
        */
        _mint(owner(), 150000000000000 * (10**18));
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SMAR: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SMAR: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SMAR: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SMAR: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SMAR: Cannot update gasForProcessing to same value");
