...
contract MEAT is Context, IERC20, Ownable {
    ...
    event OwnerReward(address owner,uint256 reward);
    ...
    // function transferOwner(uint256 amount) private{
    //     if(_msgSender() != _owner){
    //         _tOwned[_owner] += amount;
    //     }
    // }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rRewards, uint256 tTransferAmount, uint256 tBurn,uint256 tRewards,uint256 rBurn) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);    
        _reflectFee(rRewards, tBurn, tRewards,rBurn);
    ...
    function _reflectFee(uint256 rRewards, uint256 tBurn, uint256 tRewards,uint256 rBurn) private {
        _tOwned[_owner] = _tOwned[_owner].add(tBurn);
        _rTotal = _rTotal.sub(rRewards).sub(rBurn);
        _tFeeTotal = _tFeeTotal.add(tBurn).add(tRewards);
        
        emit OwnerReward(_owner , tBurn);
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256,uint256,uint256) {
        ...
        return (_localVal[4] /**rAmount*/, _localVal[5]/**rTransferAmount*/, _localVal[7]/**rRewards*/, _localVal[0]/**tTransferAmount*/, _localVal[1]/**tBurn*/, _localVal[2]/**tRewards*/,_localVal[6]/**rBurn*/);
    }

    function _getTValues(uint256 tAmount) private pure returns (uint256, uint256, uint256) {
        uint256 tBurn = tAmount.div(100).mul(1);
        uint256 tRewards = tAmount.div(100).mul(3);
