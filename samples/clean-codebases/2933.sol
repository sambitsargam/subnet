...
contract StrategyGenLevCompFarmWETH is BaseStrategy {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    ...
    address payable private constant weth = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    ...
    management 
    {
        want.approve(address(flashLoanPlugin), 0);
        flashLoanPlugin = IFlashLoanPlugin(_plugin);
        want.approve(_plugin, uint256(-1));
    }
    ...
    function useLoanTokens(
        ...
        want.transfer(address(flashLoanPlugin), repayAmount);
    }
