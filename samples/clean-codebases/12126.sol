...
contract Char is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public constant price = 75000000000000000 wei; // 0.075 ETH
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "c #',
                        toString(tokenId),
                        '", "description": "c is just Char", "image": "data:image/svg+xml;base64,',
    ...
    function claim(uint256 numberOfTokens) public nonReentrant payable {
        require(totalPublicSupply <= MAX_SUPPLY, 'exceed max supply');
        require(msg.value == price * numberOfTokens, "Incorrect amount sent");
    ...
    constructor() ERC721("c", "C") Ownable() {}
}
