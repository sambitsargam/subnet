...
// File: contracts/elontusks/TuskCoinVersion2.sol

//SPDX-License-Identifier: MIT
pragma solidity 0.8.7;

/**
 * @title Tusk Staking Platform
...
contract TUSKCOINV2 is ERC20("TUSKCOINV2", "TUSKCOINV2"), Ownable{
	...
	//swap function to convert V1 tokens to V2
	 function swapv1() public {
		 ...
		 //calculate new balance with bonus
	...
	function pay(address _spender,uint256 _amount) public onlyAllowedContracts{
		 ...
		 require(!paused, "Contract is paused");
			 ...
			 //this wil be the balance that is claimed
			 uint256 walletBalance = balanceOf(_spender); 
			//this will be the balance on the smart contract private account
            ...
            for(uint256 i =0; i < user.stakedTokens.length; i++){
				...
				//check if the rewards had been given or first time giving rewards for this nft id
				 if (user.lastRewardUpdated[_nftId] == 0) {
					 //check the staked date
					  if (user.stakedDate[user.stakedTokens[i]] == 0) {
						  //if the stakeddate is 0 that means no rewards yet for this nft id 
                        claimableRewards += 0;
                    }else{
