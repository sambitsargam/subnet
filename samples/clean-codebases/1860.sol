...
contract DividendPayingToken is Context, Ownable, ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    function distributeDividends(uint256 amount) public {
        ...
        uint256 received = (IERC20(rewardToken).balanceOf(address(this))).sub(balBefore);
        
        if (received > 0) {
            magnifiedDividendPerShare = magnifiedDividendPerShare.add(
                (received).mul(magnitude) / totalSupply()
            );
            emit DividendsDistributed(_msgSender(), received);

            totalDividendsDistributed = totalDividendsDistributed.add(received);
        }
    }
...
contract ERC20DividendToken is Context, ERC20, Ownable {
    ...
    uint256 public communityFundSellFee;
    ...
    uint256 public communityFundPending;
    ...
    event SwapAndSendFeeToAddress(address indexed recipient, uint256 tokensSwapped, uint256 amount);
    ...
    constructor(string memory name_, string memory symbol_, uint256 totalSupply_, address routerV2_, address rewardToken_) ERC20(name_, symbol_) {
        setBuyFees(1, 2, 2, 5);
        setSellFees(1, 2, 2, 5);

        rewardToken = rewardToken_;
        
        minTokensBeforeSwap = 100_000 * (10**18);

        maxTxAmount = totalSupply_ * (10**16); // 1%
        maxWalletAmount = totalSupply_ * 2 * (10**16); // 2%

        _deployDividendTracker(routerV2_, rewardToken_);
    ...
    function _deployDividendTracker(address _uniswapV2Router, address rewardToken_) internal {
        dividendTracker = new ERC20DividendTracker(rewardToken_);
    ...
    function setBurnWallet(address wallet) public onlyOwner {
        burnAddress = wallet;
    }
    ...
    function setCommunityFundWallet(address wallet) public onlyOwner {
        communityFundAddress = wallet;
    }

    function setLiquidityReceiver(address wallet) public onlyOwner {
        liquidityReceiver = wallet;
    }

    function setBuyFees(uint256 _burnBuyFee, uint256 _liquidityBuyFee, uint256 _charityBuyFee, uint256 _rewardsBuyFee) public onlyOwner {
        burnBuyFee = _burnBuyFee;
        liquidityBuyFee = _liquidityBuyFee;
        charityBuyFee = _charityBuyFee;
        rewardsBuyFee = _rewardsBuyFee;

        totalBuyFees = burnBuyFee.add(liquidityBuyFee).add(charityBuyFee).add(rewardsBuyFee);
    }

    function setSellFees(uint256 _burnSellFee, uint256 _liquiditySellFee, uint256 _communityFundSellFee, uint256 _rewardsSellFee) public onlyOwner {
        burnSellFee = _burnSellFee;
        liquiditySellFee = _liquiditySellFee;
        communityFundSellFee = _communityFundSellFee;
        rewardsSellFee = _rewardsSellFee;

        totalSellFees = burnSellFee.add(liquiditySellFee).add(communityFundSellFee).add(rewardsSellFee);
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (charityPending > 0) {
                swapAndSendFeeToAddress(charityAddress, charityPending);
                charityPending = 0;
            }

            if (communityFundPending > 0) {
                swapAndSendFeeToAddress(communityFundAddress, communityFundPending);
                communityFundPending = 0;
            }

            if (liquidityPending > 0) { 
                swapAndLiquify(liquidityPending);
                liquidityPending = 0;
            }

            if (balanceOf(address(this)) > 0) swapAndSendDividends(balanceOf(address(this)));
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) { // BUY
                ...
                liquidityPending += fees.mul(liquidityBuyFee).div(totalBuyFees);
                charityPending += fees.mul(charityBuyFee).div(totalBuyFees);                
                if (burnBuyFee > 0) {
                    uint256 burnTokens = fees.mul(burnBuyFee).div(totalBuyFees);
                    fees = fees.sub(burnTokens);
                ...
                liquidityPending += fees.mul(liquiditySellFee).div(totalSellFees);
                communityFundPending += fees.mul(communityFundSellFee).div(totalSellFees);

                if (burnSellFee > 0) {
                    uint256 burnTokens = fees.mul(burnSellFee).div(totalSellFees);
                    fees = fees.sub(burnTokens);
                    super._transfer(from, burnAddress, burnTokens);
                }
            }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        swapTokensForERC20(tokens);
        uint256 dividends = IERC20(rewardToken).balanceOf(address(this));

        bool success = IERC20(rewardToken).approve(address(dividendTracker), dividends);

        if (getNumberOfDividendTokenHolders() > 0 && success) {
            dividendTracker.distributeDividends(dividends);
            emit SendDividends(tokens, dividends);
        }
    }

    function swapAndSendFeeToAddress(address recipient, uint256 tokens) private {
        ...
        bool success = IERC20(rewardToken).transfer(recipient, dividends);

        if(success) {
            emit SwapAndSendFeeToAddress(recipient, tokens, dividends);
        }
    }
}
...
contract ERC20DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(address rewardToken_) DividendPayingToken(rewardToken_, "Dividend_Tracker", "Dividend_Tracker") {
        claimWait = 60 minutes;
...
contract OZZY is ERC20DividendToken {
    ...
    address private _burnAddress = 0x6855FB893a4088637f86663197a3Af0F395970D3;
    address private _charityAddress = 0x4B71Db09946f8676ABa8002a1a4d67e98c463EBA;
    address private _communityFundAddress = 0x66C6F9e48A3E203A06Fc07bb8ac8676C75163Ae4;
    address private _liquidityReceiver = 0xbb3e6Da084C5C05427557Fa719EeF8eC65783d81;
    // address private _owner = 0x0e83e9a254450b14D75e4e47F2fa82211f0e2647;

    constructor (address _routerAddress, address _rewardToken) ERC20DividendToken("Faded & Famous", "OZZY", _tokenSupply, _routerAddress, _rewardToken) {
        setBurnWallet(_burnAddress);
        setCharityWallet(_charityAddress);
        setCommunityFundWallet(_communityFundAddress);
        setLiquidityReceiver(_liquidityReceiver);
    }
}