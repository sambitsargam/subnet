...
// Waifu with Governance.
contract Waifu is BEP20('WAIFU', 'WAIFU') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "CoinToken::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "CoinToken::delegateBySig: invalid nonce");
        require(now <= expiry, "CoinToken::delegateBySig: signature expired");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying CoinToken (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "CoinToken::_writeCheckpoint: block number exceeds 32 bits");
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Waifu
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accMSCPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accMSCPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Waifu to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Waifu distribution occurs.
        uint256 accEggPerShare;   // Accumulated Waifu per share, times 1e12. See below.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _waifuPerBlock,
        uint256 _startBlock
    ) public {
        egg = _Waifu;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        eggPerBlock = _waifuPerBlock;
    ...
    // Update the given pool's CoinToken allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending CoinToken on frontend.
    ...
    // Safe egg transfer function, just in case if rounding error causes pool to not have enough CoinToken.
    ...
    function updateEmissionRate(uint256 _waifuPerBlock) public onlyOwner {
        massUpdatePools();
        require(_waifuPerBlock < 25000000000000000000);
        eggPerBlock = _waifuPerBlock;
    }
}