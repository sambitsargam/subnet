...
contract KIDBUU is ERC20 {
    ...
    GOTENKSDividendTracker public dividendTracker;

    address public liquidityWallet;
    address payable public TaxWallet = 0x34D05fa0792AEE5652014c41658cD73b9Cbf22DA;
    ...
    constructor() public ERC20("Kid Buu Inu", "KIDBUU") {
        uint256 _ETHRewardsFee = 1;
        uint256 _liquidityFee = 15;
    	...
    	dividendTracker = new GOTENKSDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        GOTENKSDividendTracker newDividendTracker = GOTENKSDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the KIDBUU token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "KIDBUU: Account is already the value of 'excluded'");
    ...
    function changeTaxWallet(address payable newAddress) external onlyOwner {
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "KIDBUU: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "KIDBUU: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "KIDBUU: Cannot update gasForProcessing to same value");
...
contract GOTENKSDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("KIDBUU_Dividend_Tracker", "KIDBUU_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "KIDBUU_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "KIDBUU_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main KIDBUU contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "KIDBUU_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "KIDBUU_Dividend_Tracker: Cannot update claimWait to same value");
