...
contract Ownable is Context {
     ...
     */
    constructor() {
        _owner =0x40E950a77d405CF94005D4E345C9e4f86C94D28B;
...
contract SAVIOURS is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "SAVIOURS";
    string private _symbol = "SAVI";
    ...
    uint256 internal _tokenTotal = 100_000_000_000 * (10**decimals());
    ...
    uint256 public _marketingFee=200;// meanse 2% tokens into marketingAddress
    
    address marketingAddress=0x6a80375006B78dB5190CE528D9d4e73363b96A50;
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_marketingFee != 0){
            uint256 buyBackFee = amount.mul(_marketingFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(buyBackFee);
            _reflectionBalance[marketingAddress] = _reflectionBalance[marketingAddress].add(buyBackFee.mul(rate));
            if (_isExcluded[marketingAddress]) {
                _tokenBalance[marketingAddress] = _tokenBalance[marketingAddress].add(buyBackFee);
            }
            emit Transfer(account,marketingAddress,buyBackFee);
        }
