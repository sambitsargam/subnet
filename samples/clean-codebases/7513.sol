...
Tetranote: The (un)official Tetranode token

Tax for Buying/Selling: 13%
    - 1% of each transaction sent to holders as ETH transactions
    - 6% of each transaction sent to Treasury Wallet
    - 6% of each transaction sent to the Liquidity Pool
...
contract Tetranote is Ownable, IERC20 {
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "Tetranote: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "Tetranote: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "Tetranote: Sender is blacklisted");
        require(!isBlacklisted[recipient], "Tetranote: Recipient is blacklisted");

        require(sender != address(0), "Tetranote: transfer from the zero address");
        require(recipient != address(0), "Tetranote: transfer to the zero address");
        ...
        require(
            ...
            "Tetranote: transfer amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "Tetranote: approve from the zero address");
        require(spender != address(0), "Tetranote: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "Tetranote: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    }
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "Tetranote: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "Tetranote: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "Tetranote: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "Tetranote: DEX pair can not be removed");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "Tetranote: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "Tetranote: the router is already set to the new address"
        );
    ...
    function compound() public {
        require(compoundingEnabled, "Tetranote: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "Tetranote_DividendTracker";
    string private _symbol = "Tetranote_DividendTracker";
    ...
    constructor(address _tokenAddress, address _uniswapRouter) {
        minTokenBalanceForDividends = 10000000 * (10**18);
        ...
        onlyOwner
    {
        require(
            ...
            "Tetranote_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "Tetranote_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "Tetranote_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "Tetranote_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("Tetranote_DividendTracker: method not implemented");
    }
