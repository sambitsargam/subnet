...
contract AERIUS is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Aerius Protocol";
    string private _symbol = "AERP";
    ...
    uint256 internal _tokenTotal = 6_000e9;
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "AERIUS: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'AERIUS: The contract it self cannot be excluded');
        require(!_isExcluded[account], "AERIUS: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "AERIUS: Account is already included");
