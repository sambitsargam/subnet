...
contract Ownable is Context {
    ...
    constructor() internal {
        ...
        _owner = 0x514018744DA659E750877162D0387e580a8D00d0;
...
contract EthereumSafe is Context, IERC20, Ownable {
    ...
    uint256 public _burnFee = 2; // means 3% it menas on burn 
    
    uint256 public ownerFee = 200; //200 means 2% will send to charity
    ...
    uint256 roundValue = value.ceil(ownerFee);
    uint256 onePercent = roundValue.mul(ownerFee).div(10000);
    return onePercent;
  }
    ...
    function _transfer(
        ...
        if(isFeeActive && !isTaxless[_msgSender()] && !isTaxless[recipient] && !inSwapAndLiquify){
            ...
            _reflectionBalance[ownerAddress] = _reflectionBalance[ownerAddress].add(tokensToBurn.mul(rate));
        
            emit Transfer(sender, ownerAddress, tokensToBurn);
        }
