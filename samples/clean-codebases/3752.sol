...
contract KONGKONG is Context, IERC20, Ownable {
    ...
    string private constant _name = "KONG KONG";
    string private constant _symbol = "$KK";
    ...
    mapping(address => bool) public blacklist; // mapping to track addresses on blacklist
    uint256 public blacklistEnd; // deadline to add to blacklist
    ...
    event Blacklist(address indexed sniper);
    ...
    constructor() {
        ...
        maxTransactionAmount = _tTotal * 5 / 1000 + 100000000; // 0.5% maxTransactionAmountTxn +100000000 to fix uniswap error
        minimumTokensBeforeSwap = _tTotal * 5 / 10000; // 0.05% swap tokens amount
        
        marketingAddress = payable(0x43b5E9670187764e231521077f035bBbB0F56D48); // Marketing Address
        
        devAddress = payable(0xEB7d0bE769Cf857aFa5626B5F8208528F034b1Da); // Dev Address
        
        liquidityAddress = devAddress; // Liquidity Address (switches to dead address once launch happens)
    ...
    function enableTrading() internal onlyOwner {
        ...
        blacklistEnd = block.timestamp + 20 minutes;
    }
    ...
    function _transfer(
        ...
        if(limitsInEffect){
            if (
                ...
                //blacklist if buy before active block
                if(!tradingActive){
                    blacklist[to] = true;
                    emit Blacklist(to);
                }
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            // Sell
                ...
                require(!blacklist[from], "Snipers can't sell tokens. DM a Mod.");
            // Normal transfers do not get taxed
            } else {
                require(!blacklist[from], "Snipers can't transfer tokens. DM a Mod.");
                removeAllFee();
                buyOrSellSwitch = TRANSFER; // TRANSFERs do not pay a tax.
            }
        }
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 ethForDev= ethForMarketing * 2 / 12; 
    ...
    function addToBlacklist(address account) external onlyOwner {
        require(block.timestamp <= blacklistEnd, "Cannot blacklist after 20 minutes.");
    ...
    function setMaxTransactionAmount(uint256 _maxTransactionAmount) public onlyOwner {
        require(_maxTransactionAmount > maxTransactionAmount, "Max transaction can only be increased.");
