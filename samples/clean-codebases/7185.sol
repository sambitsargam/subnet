...
DEVIENT NETWROK Socials:
Twitter: https://twitter.com/devient_network
Website: https://devient.io/
Whitepaper: https://devient.io/wp-content/uploads/2021/11/DevientNetwork_Whitepaper_V1-2.pdf

** Using FTP LPAdd to recycle 9.00% of ALL transactions back into the liquidity pool. **
...
contract Taxable is Ownable {
    ...
    function initTax() internal virtual {
		...
		setTaxAlloc(payable(0x7412a6809cD1A5Cc0Cb4a191886903Cb74d2413E), 4500);
		setTaxAlloc(payable(0xCc6a863E5B5b5C6e41E2439F85c01556D355edA9), 500);
        m_DidDeploy = true;
    }
...
contract DEVIENTNETWROK is Context, IERC20, Taxable {
    using SafeMath for uint256;
    // TOKEN
    uint256 private constant TOTAL_SUPPLY = 1000000000 * 10**9;
    string private m_Name = "DEVIENT NETWROK";
    string private m_Symbol = "DVE";
	...
	// LP ADD
    IWETH private WETH;
    uint256 private m_LiqAlloc = 9000;
    // MISC
    ...
    function _checkTX() private view returns (uint256){
        if(block.timestamp <= m_Launched.add(4 minutes))
            return TOTAL_SUPPLY.div(67);
        else
            return TOTAL_SUPPLY;
    }
