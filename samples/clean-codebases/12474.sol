...
// File contracts/CudlWeapons.sol
...
contract CudlWeapons is ERC721Enumerable, ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;
    // OG mooncat contract
    IERC721 mooncat = IERC721(0xc3f733ca98E0daD0386979Eb96fb1722A1A05E69);
    ...
    event BoostCoefficientUpdated(
        uint256 oldBoostCoefficient,
        uint256 newBoostCoefficient
    );
    ...
    function getDurability(uint256 tokenId)
        public
        view
    ...
    function getScore(
        ...
        uint256 rando = uint256(
            keccak256(abi.encodePacked(output, toString(tokenId)))
        ) % 100;
        uint256 score;
        if (rando <= 10) {
            score += 10;
        ...
        returns (string memory)
    {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="#7d5cff" /><text x="10" y="20" class="base">';
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Weapon #',
                        toString(tokenId),
                        '", "description": "The Weaponsmith is open. Cudlers will be able to mint their weapons and reveal their underlying powers - attack, defense, durability, weight and magic. Mint your weapon, reveal your stats.", "image": "data:image/svg+xml;base64,',
    ...
    function claimForMooncat(uint256 tokenId) public nonReentrant {
        require(tokenId > 0 && tokenId < 25440, "Token ID invalid");
        require(mooncat.ownerOf(tokenId) == msg.sender, "Not mooncat owner");
        _safeMint(_msgSender(), tokenId);
    }

    function claim(uint256 tokenId) public payable nonReentrant {
        require(tokenId > 25440 && tokenId < 33000, "Token ID invalid");
    ...
    function ownerClaim(uint256 tokenId) public nonReentrant onlyOwner {
        require(tokenId > 33000 && tokenId < 33200, "Token ID invalid");
    ...
    constructor() ERC721("Cudl Weapon", "CWEAPON") Ownable() {
