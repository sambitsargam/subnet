...
contract NovaxToken is ERC20 ,Ownable {
    ...
    mapping(uint256 => uint) public months; // new 12 month from this month as array
    ...
    function unlockSuperLock() public {
         ...
         // diff Months From Start Date To Last Withdraw Date
         uint deff  = BokkyPooBahsDateTimeLibrary.diffYears(StartDate[sender],now);
         
         // if rerequest before 1 year from start lock
         require(deff > 1, "Your SuperLock duration has not completed 1 year!");
         
         // earnings amount must be 
