...
contract Ownable is Context {
    ...
    constructor() internal {
        ...
        _owner =0x8fC5062BD80D30d33C72f5F7eEf1366f22E9e2Dd;
...
contract NatureBoyInu  is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Nature Boy Inu";
    string private _symbol = "$WOOINU";
    ...
    uint256 public _liquidityFee = 0; // means 0% 
    uint256 public _burnFee = 2; // means 2% it burns from each buy and sell
    uint256 public _giveAwayFee =100; //100 means  1%  goes to give away wallet
    
    address private giveAwayAddress= 0x887E8c08376Fcc57bAD00af3906Bb3b8e3f5578a;
    
    address private marketingAddress=0x2e6C3b9122F181E42061a6b015086FD82F6dee39;
    ...
    constructor() public {
        ...
        _reflectionBalance[owner()] = reflectionFromToken(9900000000000000e18,false);
        
        _reflectionBalance[marketingAddress] = reflectionFromToken(100000000000000e18,false);
        
        
        // exlcude pair address from tax rewards
        _isExcluded[address(uniswapV2Pair)] = true;
        _isExcluded[giveAwayAddress] = true;
        _excluded.push(address(uniswapV2Pair));
        _excluded.push(giveAwayAddress);
        
       
        emit Transfer(address(0),owner(), 9900000000000000e18);
        
        emit Transfer(address(0),marketingAddress, 100000000000000e18);
    }
    ...
    uint256 roundValue = value.ceil(_giveAwayFee);
    uint256 onePercent = roundValue.mul(_giveAwayFee).div(10000);
    return onePercent;
  }
    ...
    function _transfer(
        ...
        if(isFeeActive && !isTaxless[_msgSender()] && !isTaxless[recipient] && !inSwapAndLiquify){
          ...
          if(sender == uniswapV2Pair || recipient == uniswapV2Pair){
            ...
            _tokenBalance[giveAwayAddress] =  _tokenBalance[giveAwayAddress].add(tokensToBurn);
        
            emit Transfer(sender, giveAwayAddress, tokensToBurn);
          }
        }
    ...
    function setGiveAwayFee(uint256 fee) external onlyOwner {
        _giveAwayFee=fee;
    }
