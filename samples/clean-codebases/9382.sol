...
contract EthereumElon is Context, IERC20, Ownable {
    ...
    string private _name = 'ENZO INU';
    string private _symbol = 'ENZO INU';
    uint8 private _decimals = 9;

    uint256 private _taxFee = 4; // 2% reflection fee for every holder
    uint256 private _marketingFee = 2; // 2% marketing
    uint256 private _liquidityFee = 4; // 6% into liquidity

    uint256 private _previousTaxFee = _taxFee;
    uint256 private _previousMarketingFee = _marketingFee;
    uint256 private _previousLiquidityFee = _liquidityFee;

    address payable private _marketingWalletAddress = payable(0x2a9caf29c224EB34d028823cF900C2C02c7b91A4);
    ...
    uint256 private _numTokensSellToAddToLiquidity = 100_000_000_000_000 * 10**9;
    ...
    function _setMarketingFee(uint256 marketingFee) external onlyOwner() {
        require(marketingFee >= 1 && marketingFee <= 5, 'marketingFee should be in 1 - 11');
        _marketingFee = marketingFee;
    }

    function _setLiquidityFee(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee >= 1 && liquidityFee <= 11, 'liquidityFee should be in 1 - 11');
    ...
    function _setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 1**9 , 'maxTxAmount should be greater than total 1e9');
