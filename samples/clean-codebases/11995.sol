...
contract Fuxi is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Fuxi #', toString(tokenId), '", "description": "Fuxi was the creator of gossip.\n Fuxi\'s First Era created 8,888 NFTs, ranging from #1 to #8888. You can randomly make a gossip NFT based on the address code, and then start divination, battle and other gameplay.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
