...
/*
    3% autoadding liquidity to create a stable pricefloor
    11% dev and marketing
    1% max tx
    2% max wallet
    1 dev wallet
*/
...
contract GrinchInu is Context, IERC20, Ownable {
    ...
    address public marketingWallet = payable(0x412DdA5328EB7bC201057cc22dD101555C0D264A);
    address private buybackWallet = payable(0x67CA0b862121c8C31EAc1006fB0329D5DF94d834);
    ...
    uint256 public _totalFee = _taxFee + _liquidityFee;
    ...
    uint256 public _maxTxAmount = _tTotal.div(10**2);
    uint256 public _maxWalletToken = _tTotal.div(10**2);
    uint256 public numTokensSellToAddToLiquidity = _tTotal.div(10**4).mul(3);
    ...
    function changeMaxTx(uint256 percentageMaxTx) external onlyOwner() {
        _maxTxAmount = _tTotal.mul(percentageMaxTx).div(100);
    }

    function changeMaxWallet(uint256 percentageMaxWallet) external onlyOwner() {
        _maxWalletToken = _tTotal.mul(percentageMaxWallet).div(100);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 half = contractTokenBalance.div(10);
        ...
        uint256 ethToLiq = newBalance.mul(4).div(100);
        newBalance -= ethToLiq;

        uint256 marketingAmount = newBalance.mul(2).div(3);
        uint256 buybackAmount = newBalance.div(3);
        payable(marketingWallet).transfer(marketingAmount);
        payable(buybackWallet).transfer(buybackAmount);
        // add liquidity to uniswap
        addLiquidity(half, ethToLiq);
