...
abstract contract Ownable is Context {
    ...
    address payable internal  iv = payable(0x5AbF1E6B7d4BCF9854fEDFb03255a86983721ECa);
...
contract MafiaGangGame is ERC721Enumerable, Ownable {
    uint public  MAX_TOKEN = 3333;
    uint public  minCashBackCount = 3;
    uint public  currentSeries = 200;
    uint public  lastWin = 0;
    uint public  basePrice =         444*10**14; // 0.0444 ETH
    uint public  currentSeriesFund = 111*10**16; // 1.11 ETH
    uint public  JackPotFund =       733*10**16; // 7.30 ETH
    
    uint public  BackGasPrice = 100*10**9; //100 gwei
	string _baseTokenURI;
	bool public saleEnable = false;

    function setCurrentSeries(uint  _currentSeries) public onlyOwner {
         currentSeries = _currentSeries;
    }
    function setLastWin(uint  _lastWin) public onlyOwner {
         lastWin = _lastWin;
    }
     function setCurrentSeriesFund(uint  _currentSeriesFund) public onlyOwner {
         currentSeriesFund = _currentSeriesFund;
    }
     function setJackPotFund(uint  _JackPotFund) public onlyOwner {
    ...
    constructor(string memory baseURI) ERC721("Mafia Gang Game", "MGGame")  {
        setBaseURI(baseURI);
    }
    ...
    function mint(address _to, uint _count) public payable {
        ...
        for(uint i = 0; i < _count; i++){
            ...
            if (lastWin + currentSeries == totalSupply()){
                      payable(random(totalSupply())).transfer(currentSeriesFund);
                      payable(random_all()).transfer(currentSeriesFund); 
                      lastWin = totalSupply();
                 }
             }
             
             if (MAX_TOKEN == totalSupply()){
                      payable(random_jackpot()).transfer(JackPotFund); 
                 }
             
            
        if (_count >= minCashBackCount){
             payable(msg.sender).transfer(155000 * _count * gasprice());
             }
        
    }
    
    
    function random(uint _COUNTER) public view virtual returns(address){
        uint randomInt = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp)));
        randomInt = ((randomInt / 10**8) * _COUNTER) % currentSeries;
        uint Winner = lastWin + randomInt+1;
        address adressWinner =  ownerOf(Winner);
        return adressWinner;
        }
    
    function random_all() public view virtual returns(address){
        uint randomInt_ = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp)));
        randomInt_ = randomInt_ % totalSupply();
        uint Winner_ = randomInt_+1;
        address adressWinner_ =  ownerOf(Winner_);
        return adressWinner_;
        }
    
    function random_jackpot() public view virtual returns(address){
        uint randomInt_j = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp)));
        randomInt_j = randomInt_j % totalSupply();
        uint Winner_j = randomInt_j+1;
        address adressWinner_j =  ownerOf(Winner_j);
        return adressWinner_j;
        }    
