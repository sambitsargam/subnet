...
contract CryptoCrab is ERC721, ERC721Enumerable, Ownable {
    ...
    function internalMint(address to, uint256 tokenId, bytes1 generation, bytes1 reserve) internal {
        ...
        for(uint i=12; i <= 22; i++){
            ...
            if( i > 17 ){
                ...
                if(byteResult > 0xe0){
                    unchecked {
                        //Shell and background attributes can evolve twice so take off a bit more
                        byteResult = (byteResult ^ (byteResult << 4))-0x10;
                    }
                    if(byteResult > uint8(setCrabyDNA[i])){byteResult = 0x00;}
                    setCrabyDNA ^= (x & bytes1(byteResult)) >> (i*8);
                }
    ...
    //Check how many have been minted
    ...
    // Feed that crab!
    function feed(uint256 crab) public{
    ...
    // Play with the crab!
    function play(uint256 crab) public{
        require(_owns(msg.sender, crab), "!OWN");
    ...
    // Groom the crab!
    function groom(uint256 crab) public{
        require(_owns(msg.sender, crab), "!OWN");
    ...
    /*--------------------------------------------*/
    //        Evolve crabs for gas                //
    /*--------------------------------------------*/
    function evolve(uint256 crab) external{
        require(_owns(msg.sender, crab), "!OWN");
    ...
    function breed(uint256 oneCrab, uint256 anotherCrab) external payable {
        ...
        // this will be the new crabs DNA
        ...
        for (uint8 i = 12; i < 15; i++){
            if (dnaMixer[i] > 0x8F){
                offspringDNA ^= (x & oneCrabyDNA[i+11]) >> (i*8);
            }
             else{
                offspringDNA ^= (x & anotherCrabyDNA[i+11]) >> (i*8);
            }
        }

        for (uint8 i = 18; i < 23; i++){
            if (dnaMixer[i] > 0x8F){
                offspringDNA ^= (x & oneCrabyDNA[i+8]) >> (i*8);
            }
             else{
                offspringDNA ^= (x & anotherCrabyDNA[i+8]) >> (i*8);
            }
        }
        ...
        //Mint the offspring
        uint256 tokenId = maxSupply + bredCrabs + 1;
        bredCrabs += 1;  
