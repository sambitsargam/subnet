...
Sherman Capital has joined the cryptoverse, the first venture capital on the ERC platform.

We have been following the latest trends in the different blockchains and have decided to settle on ETH due to the credibility and operability of the chain.
This is our first venture into the crypto space and we have partnered with a team of developers recently hired to start up a venture capital fund.
Purchase $SC to hold a slice of the venture fund as we invent new ways to invest through:

1. Using funds for IDOs (Any chains, developers are assisting with bridges)
2. Holding huge wallets in small cap projects with utility (no meme), buying out wallets
3. Provision of assistance to these projects that we have participation in
4. DeFi projects in the lunaverse

As our abode is in Hongkong, HK is clamping down on digital assets hence we are not referencing any crypto links on our main website.

Sherman.capital -> for credibility
Shermancapital.finance will be the dapp that will go live by end of day
https://t.me/ShermanCapital
...
contract ShermanCapital is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(uint256 liqFee, uint256 marketFee, uint256 supply, uint256 maxBuyPercent, uint256 maxSellPercent, uint256 maxWalletPercent, address marketingWallet, address liqudityWallet, address uniswapV2RouterAddress) ERC20("Sherman Capital", "SC") {
