...
// File contracts/interface/ICC.sol
...
// File contracts/interface/IBoostingChef.sol


pragma solidity 0.6.12;

interface IBoostingChef {
    function proxyUnlock(uint256 _pid, uint256 _lpAmount, address userAddress) external;
    function proxyDeposit(uint256 _pid, uint256 _lpAmount, uint256 _boostAmount, address userAddress) external;
}


// File contracts/boosting/BoostingChef.sol
...
contract BoostingChef is Ownable, IBoostingChef {
    ...
    function emergencyWithdraw(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 lpAmount = user.lpAmount.sub(user.proxyLPAmount);
        require(lpAmount > 0, "Withdraw: insufficient user controlled LP balance");
        uint256 boostAmount = user.boostAmount;
        user.lpAmount = user.proxyLPAmount;
        user.boostAmount = 0;
        user.rewardDebt = 0;
        pool.totalLPAmount = pool.totalLPAmount.sub(lpAmount);
        uint256 share = calculateShare(pool.boostRate, lpAmount, boostAmount);
        pool.totalShare = pool.totalShare.sub(share);

        pool.lpToken.safeTransfer(address(msg.sender), lpAmount);
        boostToken.safeTransfer(address(msg.sender), boostAmount);
        emit EmergencyWithdraw(msg.sender, _pid, lpAmount, boostAmount);
    }
