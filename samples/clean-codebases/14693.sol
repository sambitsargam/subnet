...
contract TheDragonClub is ERC721A, Ownable, IERC2981
{
    ...
    string public collectionName = "THE DRAGON CLUB OFFICIAL";
    string public collectionSymbol = "TDC";
    ...
    uint256 public maxWhitelistAmount = 2;
    uint256 public maxPublicAmount = 10;
    uint256 public maxAmountPerOwner = 100;
    uint256 public maxSupply = 4444;
    uint256 royaltyFee = 500;
    uint256 private whitelistDate = DateTime.timestampFromDateTime(2022, 3, 29, 14, 0, 0);
    uint256 private mintDate = DateTime.timestampFromDateTime(2022, 3, 29, 16, 0, 0);
    ...
    function setWhitelistDate(uint256 _year, uint256 _month, uint256 _day, uint256 _hour, uint256 _minute, uint256 _second) public onlyOwner
    {
        whitelistDate = DateTime.timestampFromDateTime(_year, _month, _day, _hour, _minute, _second);
    }
    ...
    function setWhitelist(address[] memory addresses) public onlyOwner
    {
        ...
        for (uint i = 0; i < addresses.length; i++)
            _whitelistUsers[addresses[i]] = true;
    }
    ...
    {
        return bool(_whitelistUsers[user]);
    }
    ...
    function mint(uint256 quantity) public payable
    {
        ...
        require(block.timestamp >= whitelistDate);
        require(_totalMinted() + quantity <= maxSupply);
        require(quantity <= maxPublicAmount);
        require(balanceOf(msg.sender) < maxAmountPerOwner);

        if (block.timestamp < mintDate)
        {
            require(_whitelistUsers[msg.sender] == true);
            require(quantity <= 2);
