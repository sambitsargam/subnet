...
contract StandardPriceOracle is PriceOracle, ExponentialNoError, Ownable {
    address public cefilAddress;
    address public cmfilAddress;
    address public cfilstAddress;
    address public cusdtAddress;

    ChainlinkAggregator public filPriceFeed;
    ChainlinkAggregator public usdtPriceFeed;

    address public poster;
    uint public filstNpvPledgedValue;
    uint public filstNpvEstimatedRewards;

    event PosterChanged(address originalPoster, address newPoster);
    event NPVValueCHanged(uint originalFilstNpvPledgedValue, uint newFilstNpvPledgedValue, uint originalFilstNpvEstimatedRewards, uint newFilstNpvEstimatedRewards);

    constructor(
        address poster_,
        address cefilAddress_, 
        address cmfilAddress_, 
        address cfilstAddress_, 
        address cusdtAddress_, 
        ChainlinkAggregator filPriceFeed_, 
        ChainlinkAggregator usdtPriceFeed_
    ) public {
        setPoster(poster_);

        cefilAddress = cefilAddress_;
        cmfilAddress = cmfilAddress_;
        cfilstAddress = cfilstAddress_;
        cusdtAddress = cusdtAddress_;

        filPriceFeed = filPriceFeed_;
        usdtPriceFeed = usdtPriceFeed_;
    }

    function getUnderlyingPrice(CToken cToken) external view returns (uint) {
        address cTokenAddress = address(cToken);

        if (cTokenAddress == cefilAddress) {
            return mantissaOne;
        } else if (cTokenAddress == cmfilAddress) {
            return mantissaOne / 4 * 5 ;
        } else if (cTokenAddress == cfilstAddress) {
            uint npv = getFilstNPV();
            require(npv > 0, "npv");

            return npv;
        } else if (cTokenAddress == cusdtAddress) {
            uint usdtPriceInFil = getUsdtPriceInFil();
            require(usdtPriceInFil > 0, "usdtPriceInFil");

            return mul_(usdtPriceInFil, 10 ** 12); 
        } else {
            revert("Unknown CToken");
        }
    }

    function getUsdtPriceInFil() public view returns (uint) {
        ( ,int intFilPrice, , ,) = filPriceFeed.latestRoundData();
        require(intFilPrice > 0, "filPrice");
        uint filPrice  = uint(intFilPrice);

        ( ,int intUsdtPrice, , ,) = usdtPriceFeed.latestRoundData();
        require(intUsdtPrice > 0, "usdtPrice");
        uint usdtPrice = uint(intUsdtPrice);

        uint filPriceDecimals = filPriceFeed.decimals();
        uint usdtPriceDecimals = usdtPriceFeed.decimals();

        Double memory doubleScaledUsdtPrice = fraction(usdtPrice, 10**usdtPriceDecimals);
        Double memory doubleScaledUsdtPriceInFil = div_(mul_(doubleScaledUsdtPrice, 10**filPriceDecimals), filPrice);

        return mul_(doubleScaledUsdtPriceInFil, 10 ** 18).mantissa / doubleScale;
    }
    ...
    function setPoster(address newPoster) public onlyOwner {
        ...
        emit PosterChanged(poster, newPoster);
        // update
        poster = newPoster;
    }

    // set filstNPV
    function setFilstNPV(uint filstNpvPledgedValue_, uint filstNpvEstimatedRewards_) external {
        require(msg.sender == poster, "poster check");
        require(filstNpvPledgedValue_ != 0, "invalid new pledged value");
        require(filstNpvEstimatedRewards_ != 0, "invalid new estimated rewards");
        emit NPVValueCHanged(filstNpvPledgedValue, filstNpvPledgedValue_, filstNpvEstimatedRewards, filstNpvEstimatedRewards_);
        // update
        filstNpvPledgedValue = filstNpvPledgedValue_;
        filstNpvEstimatedRewards = filstNpvEstimatedRewards_;
        
    }

    function getFilstNPV() public view returns(uint){
        return add_(filstNpvPledgedValue, filstNpvEstimatedRewards, "getFilstNPV");
    }
}