...
contract OlympusBondDepository is Ownable {
    ...
    function redeem( bool _stake ) external returns ( uint ) {        
        ...
        if ( percentVested >= 10000 ) { // if fully vested
            ...
            emit BondRedeemed( info.payout, 0 ); // emit bond data
            return stakeOrSend( _stake, info.payout ); // pay user everything due
            ...
            emit BondRedeemed( payout, bondInfo[ msg.sender ].payout );
    ...
    function stakeOrSend( bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            ...
            if ( useHelper ) { // use if staking warmup is 0
                ...
                IStakingHelper( stakingHelper ).stake( _amount, msg.sender );
            } else {
