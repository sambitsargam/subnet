...
// MasterChef is the master of Recycle. He can make Recycle and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Recycle is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Recycles
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accRecyclePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accRecyclePerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Block number when bonus Recycle period ends.
    uint256 public bonusEndBlock;
    // Recycle tokens created per block.
    uint256 public RecyclePerBlock;
    // Bonus muliplier for early Recycle makers.
    ...
    // The block number when Recycle mining starts.
    ...
    // Update the given pool's Recycle allocation point. Can only be called by the owner.
...
//        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
    ...
    // Deposit LP tokens to MasterChef for Recycle allocation.
