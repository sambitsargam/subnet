...
contract CrazyCrawFish is ERC721Enumerable, Ownable
{
    using SafeMath for uint256;
    using Strings for uint256;

    uint public constant _TOTALSUPPLY = 510;
    uint public maxQuantity =10;
    uint public maxPerUser=10;
    uint public publicMint=0;
    uint256 public price = 0.08 ether; 
    uint256 public status = 0; // 0-pause,  1-public
    bool public reveal = false;
   

    constructor(string memory baseURI) ERC721("Crazy Craw Fish", "CCF") {
        setBaseURI(baseURI);
         for (uint i = 0; i < 10; i++) {
    ...
    modifier isSaleOpen{
        require(totalSupply() < 510, "Sale end");
        _;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if(bytes(base).length > 0){
            return reveal ? string(abi.encodePacked(base, tokenId.toString(), ".json")) : string(abi.encodePacked("https://gateway.pinata.cloud/ipfs/QmS9k3zNtiwY3kR2iXzjzWcqp3dWpNwmptP8dj5FwxFzjv"));
        }
        else 
        return "";
    }
    function contractURI() public view returns (string memory) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "CrazyCrawFish", "description": "Custom art pieces each granting access to exclusive NFT knowledge.", "seller_fee_basis_points": 300, "fee_recipient": "0x06921164f7604a951086Ed880CbDdbcd41D8315D"}'))));
