...
contract CANDIES is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
	
	address public _charityWallet = 0x948443F6dF61aEc77064AaC59952EA7837C156Df;
    address public _developmentWallet = 0x322e53DEe6cb35AEa181234D42698BdCAd1a69d5;
	address public _burnAddress = 0x000000000000000000000000000000000000dEaD;
	address public _stakingAddress = 0x23801Eb30E249692F38Bc074523FCeafB98a5c3D;
    ...
    string private _name = "Candyverse Coin";
    string private _symbol = "CANDIES";
    ...
    uint256 public _reflectionFee = 600;
    uint256 private _previousReflectionFee = _reflectionFee;
	
	uint256 public _burnFee = 200;
    uint256 private _previousBurnFee = _burnFee;
	
	uint256 public _stakingFee = 200;
    uint256 private _previousStakingFee = _stakingFee;
	
    uint256 private _liquidityFee = liquidityFee.add(_developmentFee).add(_charityFee);
    ...
    uint256 public _maxSaleTxAmount = 1250000000000 * (10**9);
	uint256 public _maxWalletAmount = 1000000000000000 * (10**9);
    ...
    constructor () {
		...
		_isExcludedFromMaxWalletToken[_stakingAddress] = true;
    ...
    function setReflectionFeePercent(uint256 newFee) external onlyOwner() {
	   require(_stakingFee.add(_burnFee).add(liquidityFee).add(_developmentFee).add(_charityFee).add(newFee) < 10000, "Percentage must be less than total transfer amount");
       _reflectionFee = newFee;
    }
	
	function setStakingFeePercent(uint256 newFee) external onlyOwner() {
	   require(_reflectionFee.add(_burnFee).add(liquidityFee).add(_developmentFee).add(_charityFee).add(newFee) < 10000, "Percentage must be less than total transfer amount");
        _stakingFee = newFee;
    }
	
	function setBurnFeePercent(uint256 newFee) external onlyOwner() {
	   require(_reflectionFee.add(_stakingFee).add(liquidityFee).add(_developmentFee).add(_charityFee).add(newFee) < 10000, "Percentage must be less than total transfer amount");
       _burnFee = newFee;
    }
	
	function setLiquidityFeePercent(uint256 newFee) external onlyOwner() {
	   require(_reflectionFee.add(_stakingFee).add(_burnFee).add(_developmentFee).add(_charityFee).add(newFee) < 10000, "Percentage must be less than total transfer amount");
       liquidityFee = newFee;
    }
	
	function setDevelopmentFeePercent(uint256 newFee) external onlyOwner() {
	    require(_reflectionFee.add(_stakingFee).add(_burnFee).add(liquidityFee).add(_charityFee).add(newFee) < 10000, "Percentage must be less than total transfer amount");
        _developmentFee = newFee;
    }
	
	function setCharityFeePercent(uint256 newFee) external onlyOwner() {
	   require(_reflectionFee.add(_stakingFee).add(_burnFee).add(liquidityFee).add(_developmentFee).add(newFee) < 10000, "Percentage must be less than total transfer amount");
       _charityFee = newFee;
    }
	...
	function setStakingWallet(address payable newAddress) external onlyOwner() {
	   require(newAddress != address(0), "new wallet is the zero address");
       _stakingAddress = newAddress;
    }
	...
	function _takeStaking(uint256 tStaking) private {
        ...
        if(_isExcluded[_stakingAddress])
            _tOwned[_stakingAddress] = _tOwned[_stakingAddress].add(rStaking);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
		...
		if(to == uniswapV2Pair && from != owner() && to != owner()) 
		{
		    require(amount <= _maxSaleTxAmount, "Transfer amount exceeds the _maxSaleTxAmount.");
		}
		
		if(!_isExcludedFromMaxWalletToken[to] && to != uniswapV2Pair) 
		{
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
	{
		...
		uint256 charityPart = newBalance.div(_liquidityFee).mul(_charityFee);
		uint256 developmentPart = newBalance.sub(liquidityPart).sub(charityPart);
		
		payable(_charityWallet).transfer(charityPart);
		payable(_developmentWallet).transfer(developmentPart);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(tStaking));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(tStaking));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, amount.sub(tStaking));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(tStaking));
        } else {
            _transferStandard(sender, recipient, amount.sub(tStaking));
        }
        if(!takeFee)
