...
Website: https://spaceleaguetoken.com/

Telegram: https://t.me/SpaceLeagueToken

Official SpaceLeague Token

*/

// Verified using https://dapp.tools

// hevm: flattened sources of src/SPL.sol
...
contract SpaceLeague is Ownable, IERC20 {
    ...
    constructor(){
        address _marketingWallet = address(0x015daE1Da32c581B76143f6918e87D7Ec7b74B43);
        address _liquidityWallet = address(0x015daE1Da32c581B76143f6918e87D7Ec7b74B43);
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "SPL: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "SPL: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "SPL: Sender is blacklisted");
        require(!isBlacklisted[recipient], "SPL: Recipient is blacklisted");

        require(sender != address(0), "SPL: transfer from the zero address");
        require(recipient != address(0), "SPL: transfer to the zero address");
        ...
        require(
            ...
            "SPL: transfer amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "SPL: approve from the zero address");
        require(spender != address(0), "SPL: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "SPL: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    }
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "SPL: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "SPL: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "SPL: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "SPL: DEX pair can not be removed");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "SPL: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "SPL: the router is already set to the new address"
        );
    ...
    function compound() public {
        require(compoundingEnabled, "SPL: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "SPL_DividendTracker";
    string private _symbol = "SPL_DividendTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "SPL_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "SPL_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "SPL_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "SPL_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("SPL_DividendTracker: method not implemented");
    }
