...
contract HootyNest is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    // Base Token reward bool
    bool public isBaseToken;
    ...
    // Deposit tokens to Stake for reward allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            if (!hasStaked[from]) {
                ...
                uint256 newAmount = deposits[from].depositAmount.add(_amount);
                deposits[from] = Deposits(
                    newAmount,
                    block.timestamp,
                    block.timestamp.add((lockDuration.mul(3600))),
    ...
    function _withdraw(uint256 _pid, uint256 _amount) public {
        address from = msg.sender;
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    modifier _withdrawCheck(address from) {
        ...
        require(
            block.timestamp >= deposits[from].endTime || unlocked == true,
    ...
    function _emergencyWithdraw(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function setIsBaseToken(bool _isBaseToken) external onlyOwner {
    ...
    function unlockVault(bool _locked) external onlyOwner {
        unlocked = _locked;
    }
