...
contract BabyJesus is ERC20, Ownable {
    ...
    constructor() ERC20("BabyJesus", "BABYJESUS") {
        ...
        maxWallet = maxSell * 15 / 1000; // 1.5% maxWallet
    ...
    function _transfer(
        ...
        if(from != owner() && from != presaleAddress && from != presaleRouterAddress && block.number == tradingActiveBlock && !_isExcludedMaxSellTransactionAmount[to]){
                boughtEarly[to] = true;
        }
    ...
    function buyBackTokens(uint256 ethAmount) external onlyOwner {
