...
contract ThirdWorldInu is IBEP20, Auth {
    using SafeMath for uint256;

    event TradingEnabled(uint256 _startDate);


    address constant private WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2; //Mainnet BSC :0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c // Testnet BSC: 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd // ETH Mainnet: 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D // ETH Testnet: 0xc778417E063141139Fce010982780140Aa0cD5Ab
    address constant private DEAD = 0x000000000000000000000000000000000000dEaD; 
    address constant private ZERO = 0x0000000000000000000000000000000000000000;

    string private constant _name = "Third world inu";
    ...
    uint256 private tempCharityFee;
    ...
    address private charityFeeReceiver = 0xE1B48FC06A29aA106b7ca6bf937788700a146F02;
    ...
    uint256 public swapThreshold = _totalSupply / 1000 * 1; // 0.5%
    ...
    function getSwapFees() internal {
        ...
        tempCharityFee = charityFee.add(sellCharityFee).div(2);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHCharity = amountETH.mul(tempCharityFee).div(totalETHFee);
        uint256 amountETHMarketing = amountETH.mul(tempMarketingFee).div(totalETHFee);
        
       
        (MarketingSuccess, /* bytes memory data */) = payable(marketingFeeReceiver).call{value: amountETHMarketing, gas: 30000}("");
        (DevSuccess, /* bytes memory data */) = payable(devFeeReceiver).call{value: amountETHDev, gas: 30000}("");
        (CharitySuccess, /* bytes memory data */) = payable(charityFeeReceiver).call{value: amountETHCharity, gas: 30000}("");
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function setBuyFees(uint256 _liquidityFee,  uint256 _marketingFee, uint256 _charityFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        charityFee = _charityFee;
      
        totalFee = _liquidityFee.add(_marketingFee).add(_charityFee).add(devFee);
        feeDenominator = _feeDenominator;
    }
    
    function setSellFees(uint256 _liquidityFee,  uint256 _marketingFee, uint256 _charityFee, uint256 _feeDenominator) external authorized {
        sellLiquidityFee = _liquidityFee;
        sellMarketingFee = _marketingFee;
        sellCharityFee = _charityFee;

        
        totalSellFee = _liquidityFee.add(_marketingFee).add(_charityFee).add(sellDevFee);
        feeDenominator = _feeDenominator;
    }
    ...
    function setCharityFeeReceiver(address _charityFeeReceiver) external authorized {
        marketingFeeReceiver = _charityFeeReceiver;
    }
