...
contract BabyWhale is ERC20, Ownable {
    ...
    address[] private _projectWallets;
    ...
    constructor(
        ...
        _projectWallets = projectWallets;

        commentProgramWallet = _projectWallets[0];
        decentralizedWallet = _projectWallets[1];
        airdropWallet = _projectWallets[2];
        awardDAOWallet = _projectWallets[3];
        teamWallet = _projectWallets[4];
    ...
    function isProjectWallets(address account) internal view returns (bool) {
        for (uint256 i = 0; i < _projectWallets.length; i++) {
            if (_projectWallets[i] == account) {
                return true;
            }
        }
        return false;
