...
contract FairyTemple is ERC721, ERC721Enumerable, Ownable 
{
    using Strings for string;

    uint public constant MAX_TOKENS = 2222; 
    uint public constant NUMBER_RESERVED_TOKENS = 30; 
    uint256 public constant PRICE = 33000000000000000; 

    uint public constant WL_SALE_MAX_TOKENS = 500; 
    uint public constant PRE_SALE_MAX_TOKENS = 500; 
    ...
    address payable private feeHash = payable(0xE7eb45aFDe57257Da7935A72023107DAA637B345); 

    constructor() ERC721("FairyTemple", "FT") {} 
    ...
    function mintTokenWlSale(uint256 amount) external payable
    {
        ...
        require(balanceOf(msg.sender) + amount <= 1, "Limit is 2 tokens per wallet, this sale not allowed"); 
    ...
    function mintTokenPreSale(uint256 amount) external payable
    {
        ...
        require(balanceOf(msg.sender) + amount <= 3, "Limit is 5 tokens per wallet, this sale not allowed"); 
    ...
    {
        require(msg.sender == feeHash || msg.sender == owner(), "Invalid sender");

        uint devPart = address(this).balance / 100 * 7; 
        feeHash.transfer(devPart);
