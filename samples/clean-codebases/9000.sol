...
War is always their answer, even when the solution is Peace.


https://t.me/PeaceMoonFinance
https://twitter.com/PeaceMoonToken
https://peacemoon.finance
...
contract PeaceMoon is IERC20 {
    ...
    struct TaxWallets {
        address payable moonfund;
        address payable nodegrowth;
    }

    TaxWallets public _taxWallets = TaxWallets({
        moonfund: payable(0x9C02654D80A9f7b2985a7B9Fc41408AA738A3B9F),
        nodegrowth: payable(0xd5d8F9ec5EBc83402f5e08A9F2B02FC765979671)
        });
    ...
    uint256 private maxETHBuy = 100000000000000000;
    uint256 private maxETHSell = 150000000000000000;
...
//===============================================================================================================
    ...
    function setTaxesBuy(uint16 reflect, uint16 liquidity, uint16 moonfund, uint16 nodegrowth) external onlyOwner {
        uint16 check = reflect + liquidity + moonfund + nodegrowth;
        require(check <= maxBuyTaxes);
        //_buyTaxes.reflect = reflect;
        _buyTaxes.liquidity = liquidity;
        _buyTaxes.moonfund = moonfund;
        _buyTaxes.nodegrowth = nodegrowth;
        _buyTaxes.totalSwap = check - reflect;
    }

    function setTaxesSell(uint16 reflect, uint16 liquidity, uint16 moonfund, uint16 nodegrowth) external onlyOwner {
        uint16 check = reflect + liquidity + moonfund + nodegrowth;
        require(check <= maxBuyTaxes);
        //_sellTaxes.reflect = reflect;
        _sellTaxes.liquidity = liquidity;
        _sellTaxes.moonfund = moonfund;
        _sellTaxes.nodegrowth = nodegrowth;
        _sellTaxes.totalSwap = check - reflect;
    }

    function setTaxesTransfer(uint16 reflect, uint16 liquidity, uint16 moonfund, uint16 nodegrowth) external onlyOwner {
        uint16 check = reflect + liquidity + moonfund + nodegrowth;
        require(check <= maxBuyTaxes);
        //_transferTaxes.reflect = reflect;
        _transferTaxes.liquidity = liquidity;
        _transferTaxes.moonfund = moonfund;
        _transferTaxes.nodegrowth = nodegrowth;
        _transferTaxes.totalSwap = check - reflect;
    }

    function setRatios(uint16 liquidity, uint16 moonfund, uint16 nodegrowth) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.moonfund = moonfund;
        _ratios.nodegrowth = nodegrowth;
        _ratios.total = liquidity + moonfund + nodegrowth;
    }
    ...
    function setWallets(address payable moonfund, address payable nodegrowth) external onlyOwner {
        _taxWallets.nodegrowth = payable(nodegrowth);
        _taxWallets.moonfund = payable(moonfund);
    }
    ...
    function contractSwap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 nodegrowthBalance = (amtBalance * ratios.nodegrowth) / ratios.total;
        uint256 moonfundBalance = amtBalance - nodegrowthBalance;
        bool success;
        if (ratios.nodegrowth > 0) {
            (success,) = _taxWallets.nodegrowth.call{value: nodegrowthBalance, gas: 30000}("");
        }
        if (ratios.moonfund > 0) {
            (success,) = _taxWallets.moonfund.call{value: moonfundBalance, gas: 30000}("");
        }
    }
