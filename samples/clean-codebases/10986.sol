...
contract MeanTamato is ERC20, Ownable {
    ...
    constructor() ERC20("Mean Tamato", "MEANTAMATO") payable {
        ...
        buyBuybackFee = 2;
        buyBurnFee = 4;
        buyTotalFees = buyOperationsFee + buyLiquidityFee + buyBuybackFee + buyBurnFee;

        sellOperationsFee = 3;
        sellLiquidityFee = 1;
        sellBuybackFee = 2;
        sellBurnFee = 4;
        sellTotalFees = sellOperationsFee + sellLiquidityFee + sellBuybackFee + sellBurnFee;
        ...
        operationsAddress = address(0x7451C0197489bd35332a5EFb6D2f21F760f0abD8);
        
        _createInitialSupply(newOwner, totalSupply*39/100);
        _createInitialSupply(address(this), totalSupply*61/100);
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _burnFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _burnFee) external onlyOwner {
    ...
    function swapBack() private {
        if(tokensForBurn > 0 && balanceOf(address(this)) >= tokensForBurn) {
            _transfer(address(this), address(0xdead), tokensForBurn);
            tokensForBurn = 0;
        }
        ...
        ethForLiquidity -= ethForOperations + ethForBuyback;
        ...
        // remainder stays on contract for buybacks
    }
    ...
    function withdrawStuckETH() external onlyOwner {
        require(!tradingActive, "Cannot withdraw ETH after launch, it is for buybacks only.");
        bool success;
        (success,) = address(msg.sender).call{value: address(this).balance}("");
    }
