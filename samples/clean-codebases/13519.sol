//Please find our telegraph group. 40% burning, 60% adding liquidity. 
//It will launch on time at 14:45 UTC today, liquidity will be locked immediately for six months at team by trust swap.
//Do not buy before locking liquidity, to prevent being cheated! 
//Our telegraph group has been established, but it has not been published yet.
//The first round of the competition offers a total of 1,000,000USDC of prizes. 
//After the bonus pool of each round is distributed, it will automatically enter to the next round, which is provided by the transaction fee of the previous round. 
//The holders who find and enter the telegram group can get the corresponding bonus. 
//The calculation formula of the bonus is as follows: 
//1000000*[holding ratio +0.1/(purchase time + time to enter the telegram group - the start time of this round)]. 
//For example: in the first round, buy 1% token 1 minute after launch, enter telegraph group 5 minutes after launch,
//can get reward: 1000000*[0.01+0.1/(1+5)]=27000USDC. 
//Tip: The telegraph group changes its entry link every 30 seconds,
//each winner who enters the telegraph group please send your Ethernet chain wallet address directly, 
//then we will send you a reward and put you out of the telegraph group to prevent cheating.
// Telegraph group into group links are all related to the ultimate answer to the universe.
...
contract FindUs is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(uint256 liqFee, uint256 marketFee, uint256 supply, uint256 maxBuyPercent, uint256 maxSellPercent, uint256 maxWalletPercent, address marketingWallet, address liqudityWallet, address uniswapV2RouterAddress) ERC20("Find Us DAO", "FUD") {
