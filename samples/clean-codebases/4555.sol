...
contract OnChainKongz { 
    ...
    function _getBio(uint256 tokenId_) public view returns (string memory) {
        ...
        string memory _compliantBio = _makeCompliantBio(_bio);

        return bytes(_bio).length == 0 ? _default : _compliantBio;
    }
    ...
    address public OCKGallery = 0xA2D007Ef9B244d5B61B8Bd282887B7AB2241aed9;
    function setOCKGallery(address gallery_) external onlyOwner {
        OCKGallery = gallery_;
    }
    function _isContract(address account_) public view returns (bool) {
        return account_.code.length > 0;
    }
    modifier onlyGallery {
        if (_isContract(msg.sender)) require(OCKGallery == msg.sender); _;
    }
    ...
    function tokenURI(uint256 tokenId_) public view 
    onlyGallery returns (string memory) {
        return _tokenURI(tokenId_);
    }
    function _raw_tokenURI(uint256 tokenId_) internal view returns (string memory) {
        string memory _metadata = string(abi.encodePacked(
            '{"name":"',
            _getName(tokenId_),
            '", "description":"',
    ...
    function raw_tokenURI(uint256 tokenId_) public view 
    onlyGallery returns (string memory) {
        return _raw_tokenURI(tokenId_);
    }
}