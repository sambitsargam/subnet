        ...
        if (sumofgenesisbear >= 5) {
            ...
            for (uint256 i = 0; i < stakes[_addr].length; i++) {
                ...
                if (isSpecialBear(stakes[_addr][i].nft_id)) {
                    ...
                    uint256 dd_srootx = calDay(stakes[_addr][i].claimedDate_SROOT);
                    uint256 dd_weth = dd_srootx;
                    claimAmountOfSROOTxToken = claimAmountOfSROOTxToken.add((dd_srootx * MaxSROOTXrate * sumofgenesisbear) / 2);
                    claimAmountOfWETH = claimAmountOfWETH.add((dd_weth * MaxSROOTXrate * sumofgenesisbear) / 2);
                }
            }
