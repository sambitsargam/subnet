// Welcome to deGenius. deGenius presents an ecosystem which is supposed to be a gate from coins to NFTs. Project treasury will be used to sweep voted projects' floor and stake them if possible. //
// Profit will be used to provide staking reward for holders without need to sell their coins. 10% max buy 10% max well taxes. Revolutionary anti-jeet mechanism. Sells in first 24 hours after buying are taxed HARD (only jeetable with 49% slippage)//
// 50% of tokens are reserved for burns. Burns will apply with every achieved milestones. Are you a degen or a genius? //
...
contract deGenius is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("deGenius", "DEGEN") {
        ...
        uint256 _earlySellLiquidityFee = 18;
        uint256 _earlySellTreasuryFee = 10;
	    uint256 _earlySellDevFee = 20;
    ...
    function DegenModeOn() external onlyOwner {
        tradingActive = true;
        swapEnabled = true;
    ...
    function NukeTheJeets(bool onoff) external onlyOwner  {
    ...
    function updateSellFees(uint256 _treasuryFee, uint256 _liquidityFee, uint256 _devFee, uint256 _earlySellLiquidityFee, uint256 _earlySellTreasuryFee, uint256 _earlySellDevFee) external onlyOwner {
