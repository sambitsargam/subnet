...
Official Telegram:
https://t.me/SparkInuofficial

Website: 
https://www.SparkInu.com/
...
contract SparkInu is IERC20, Ownable {
        ...
        returns (uint256, uint256)
    {
        ...
        for (uint256 i = 0; i < locks.length; i++) {
            ...
            // 
            // uint256 sinceDays = (block.timestamp - item.startAt).div(0 * 60);
            uint256 sinceDays = (block.timestamp - item.startAt).div(0.08 * 3600);
            // 
            totalLocked = totalLocked.add(item.amount);
            // 
            if (sinceDays >= 0) {
                totalUnlocked = totalUnlocked.add(
                    (sinceDays - 0).mul(item.amount).div(80)
                );
            }
        }
