...
contract FATOSHINFT is ERC721 {
    address payable public _contractOwner;
    uint public _dappCommision = 1;
    ...
    constructor() ERC721("FATOSHI NFT", "FATOSHINFT") {
        _contractOwner = msg.sender;
    }

    function mint(string memory _tokenURI, address _toAddress, uint _price, uint _royalty, uint _reward) public returns (uint) {

        uint _tokenId = totalSupply() + 1;
        price[_tokenId] = _price;
        royalty[_tokenId] = _royalty;
        reward[_tokenId] = _reward;
    ...
    function commision() public view virtual returns (uint) {
        return _dappCommision;
    }

    function setCommision(uint newCommision) public virtual onlyOwner {
        _dappCommision = newCommision;
    }

    function changeCommisions(uint _tokenId, uint _royalty, uint _reward) public virtual onlyOwner {
      royalty[_tokenId] = _royalty;
      reward[_tokenId] = _reward;
    }
    ...
    function _trade(uint _id) internal {
        ...
        address payable _creator = payable(creator[_id]);

        _transfer(_owner, _buyer, _id);

        uint _commissionValue = (price[_id] / 100) * _dappCommision;
        uint _creatorValue = (price[_id] / 100) * royalty[_id];
        uint _rewardValue = (price[_id] / 100) * reward[_id];
        uint _sellerValue = price[_id] - _commissionValue - _creatorValue - _rewardValue;

        _owner.transfer(_sellerValue);
        _contractOwner.transfer(_commissionValue);

        if(royalty[_id]>=1){
          _creator.transfer(_creatorValue);
        }
    ...
    function withdrawBalance() external onlyOwner {
        ...
        payable(_contractOwner).transfer(amountBNB);
    }

}

// Contract & dApp Developed by Dapprex
// www.dapprex.com