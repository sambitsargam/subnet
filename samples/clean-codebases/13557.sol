...
// File: contracts/Strategies/StrategyLenderYieldOptimiser/GenericLender/IGenericLender.sol
...
// File: contracts/Strategies/StrategyLenderYieldOptimiser/WantToEthOracle/IWantToEth.sol
...
// File: contracts/Strategies/StrategyLenderYieldOptimiser/Strategy.sol
...
contract StrategyLenderYieldOptimiser is BaseStrategy {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    ...
    function clone(
        ...
        StrategyLenderYieldOptimiser(newStrategy).initialize(_vault, _strategist, _rewards, _keeper);
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        if (bal > 0) {
            want.transfer(address(lenders[highest]), bal);
    ...
    function manualAllocation(lenderRatio[] memory _newPositions) public onlyAuthorized {
        ...
        for (uint256 i = 0; i < _newPositions.length; i++) {
            ...
            want.transfer(_newPositions[i].lender, toSend);
    ...
    function ethToWant(uint256 _amount) public override view returns (uint256) {
        // If currency is eth so no change.
