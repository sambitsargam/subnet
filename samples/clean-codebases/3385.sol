...
contract KWIK {
    ...
    uint public totalSupply = 250_000_000e18; // 250,000,000 KWIK
...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to KwikSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // KwikSwap must mint EXACTLY the same amount of KwikSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// MasterChef is the master of Kwik. He can make Kwik and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once KWIK is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of KWIKs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. KWIKs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that KWIKs distribution occurs.
        uint256 accKwikPerShare; // Accumulated KWIKs per share, times 1e12. See below.
    }
    ...
    // Block number when bonus KWIK period ends.
    uint256 public bonusEndBlock;
    // KWIK tokens created per block.
    uint256 public kwikPerBlock;
    // Bonus muliplier for early kwik makers.
    ...
    // The block number when KWIK mining starts.
    ...
    constructor(
        KWIK _kwik,
        address _devaddr,
        uint256 _kwikPerBlock,
        uint256 _startBlock,
        uint256 _bonusEndBlock
    ) public {
        kwik = _kwik;
        devaddr = _devaddr;
        kwikPerBlock = _kwikPerBlock;
    ...
    // Update the given pool's KWIK allocation point. Can only be called by the owner.
    ...
    function setBlockReward(uint _tokens) public onlyOwner {
        kwikPerBlock = _tokens;
    }

    // View function to see pending KWIKs on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 kwikReward = multiplier.mul(kwikPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        kwik.transfer(devaddr, kwikReward.div(10));
        kwik.transfer(address(this), kwikReward);
        pool.accKwikPerShare = pool.accKwikPerShare.add(kwikReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for KWIK allocation.
    ...
    function safeKwikTransfer(address _to, uint256 _amount) internal {
        uint256 kwikBal = kwik.balanceOf(address(this));
        if (_amount > kwikBal) {
            kwik.transfer(_to, kwikBal);
        } else {
            kwik.transfer(_to, _amount);
        }
    }
