...
contract HdcoreVault is OwnableUpgradeSafe {
    ...
    function addPendingRewards(uint256 _) public {
        ...
        if(newRewards > 0) {
            hdcoreBalance = hdcore.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accHdcorePerShare).div(1e12); /// This is deposited for address
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming HDCORE farmed
    ...
    function safeHdcoreTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > hdcoreBal) {
            ...
            console.log("Balance of this address is :", hdcoreBal);
        ...
        if(pending_DEV_rewards > 0) {
            ...
            safeHdcoreTransfer(devaddr, devSend);
        }
