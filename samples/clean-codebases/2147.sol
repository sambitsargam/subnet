...
// File contracts/openzeppelin_contracts/contracts/token/ERC20/IERC20.sol
...
// File contracts/openzeppelin_contracts/contracts/token/ERC20/utils/SafeERC20.sol
...
// File contracts/openzeppelin_contracts/contracts/security/ReentrancyGuard.sol
...
// File contracts/openzeppelin_contracts/contracts/security/Pausable.sol
...
contract Purchase is Whitelist,ReentrancyGuard {
    ...
    uint256 public maxTokenForOnePresaleTransaction;
    ...
    event SetTokensLimit(uint256 _forPresale,uint256 _normalSale);

    modifier validTokensAmount(uint256 _numberOfToken){
        if(isWhitelist){
            require(_numberOfToken <= maxTokenForOnePresaleTransaction,"Purchase: enter valid token amount for presale");
        }
        else{
    ...
    constructor(address _nft,address _x22,address _pair,bool _isWhitelist) public Whitelist(_isWhitelist){
        ...
        discount = 5000;
        maxTokenForOneSaleTransaction = 2;
        maxTokenForOnePresaleTransaction = 2;
    }

    function setTokensLimit(uint256 _forPresale,uint256 _normalSale) public onlyOwner(){
        maxTokenForOnePresaleTransaction = _forPresale;
        maxTokenForOneSaleTransaction = _normalSale;
        emit SetTokensLimit(maxTokenForOnePresaleTransaction,maxTokenForOneSaleTransaction);
    }

    function updateWhitelist(bool _isWhitelist) public onlyOwner(){
        isWhitelist = _isWhitelist;
        emit UpdateWhitelist(isWhitelist);
    }
    ...
    function buyWithETH(uint256 _numberOfToken) public payable nonReentrant() validTokensAmount(_numberOfToken) whenNotPaused() onlyWhitlisted(){    
    ...
    function buyWithX22(uint256 amount,uint256 _numberOfToken) public nonReentrant() validTokensAmount(_numberOfToken) whenNotPaused() onlyWhitlisted(){
        require (_numberOfToken > 0 ,"Purchase: _numberOfToken must be greater than zero");
        require(amount >= amountToPay().mul(_numberOfToken), "Purchase: amount of x22 must be equal to token price");
        x22.transferFrom(msg.sender, treasurerAddress, amountToPay().mul(_numberOfToken));
        nft.batchMint(msg.sender,_numberOfToken);
        emit BuyNFTforX22(msg.sender,amountToPay().mul(_numberOfToken),_numberOfToken);
    }
