...
contract MasterChef is WhitelistGuard
{
	...
	function pendingReward(uint256 _pid, address _account) external view returns (uint256 _pendingReward)
	{
		...
		if (block.number > _pool.lastRewardBlock && _pool.amount > 0) {
			...
			_accRewardPerShare = _accRewardPerShare.add(_reward.mul(1e12).div(_pool.amount));
		}
	...
	function updatePool(uint256 _pid) public
	{
		...
		uint256 _blockCount = _getBlockCount(_pool.lastRewardBlock, block.number);
		uint256 _reward = _blockCount.mul(rewardPerBlock).mul(_pool.allocPoint).div(totalAllocPoint);
		_pool.accRewardPerShare = _pool.accRewardPerShare.add(_reward.mul(1e12).div(_pool.amount));
		_pool.lastRewardBlock = block.number;
	...
	function deposit(uint256 _pid, uint256 _amount) external onlyEOAorWhitelist
	{
		...
		require(_pool.amount.add(_amount) <= _pool.maxAmount, "liquidity exceeded");
		updatePool(_pid);
	...
	function injectRewardFunds(uint256 _amount) external onlyOwner
	{
		IERC20(rewardToken).safeTransferFrom(msg.sender, availablePool, _amount);
	}
	...
	function recoverLostFunds(uint256 _pid) external onlyOwner
	{
		PoolInfo storage _pool = poolInfo[_pid];
		uint256 _balance = IERC20(_pool.token).balanceOf(address(this));
		IERC20(_pool.token).safeTransfer(treasury, _balance.sub(_pool.amount));
	}
	...
	function addPool(address _token, uint256 _allocPoint, uint256 _startBlock, uint256 _withdrawalUnlockTime, uint256 _maxAmount, bool _withUpdate) external onlyOwner
	{
		if (_withUpdate) {
			massUpdatePools();
		}
