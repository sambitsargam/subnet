...
*/
contract HolyPassageV4 is AccessControlUpgradeable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function getClaimableBonusForAddress(address _account) public view returns(uint256) {
        return getClaimableBonusIncludingMigration(_account, 0);
    }
    ...
    // for transfer bonus claims upon manual request and verification
    function setMigratedAmounts(address[] memory /* calldata? */ _accounts, uint256[] memory /* calldata? */ _migratedAmounts) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Admin only");
        uint256 length = _accounts.length;
        require(length == _migratedAmounts.length, "Array length mismatch");
        for (uint256 i = 0; i < length; ++i) {
            address account = _accounts[i];
            migratedTokens[account] = _migratedAmounts[i];
        }
    }

    // for transfer bonus claims upon manual request and verification
    function setClaimedBonusAmounts(address[] memory /* calldata? */ _accounts, uint256[] memory /* calldata? */ _claimedBonusAmounts) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Admin only");
        uint256 length = _accounts.length;
        require(length == _claimedBonusAmounts.length, "Array length mismatch");
        for (uint256 i = 0; i < length; ++i) {
            address account = _accounts[i];
            claimedBonusTokens[account] = _claimedBonusAmounts[i];
        }
    }
