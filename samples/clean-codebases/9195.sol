...
contract EthMenNftStaking is ReentrancyGuard, Pausable, Ownable {
    using SafeMath for uint256;
    ...
    constructor(address _NDR, address _NFT, address _NFTSTAKING) public {
        NDR = IERC20(_NDR);
        NFT = IERC1155(_NFT);
    ...
    /**
     * @dev Returns all custom NFT ids
     */
    ...
    /**
     * @dev Total staked cards in total
     */
    function totalSupply() external view returns (uint256) {
        return _totalSupply;
    }
    ...
    /**
     * @dev Total staked cards by user
     */
    ...
    /**
     * @dev Get claimable NDR amount
     */
    function earned(address account) public view returns (uint256) {
        uint8 rewardApplicable = 1;
        if(nftRequired) {
            rewardApplicable = NFTStaking.balanceOf(account) > 0 ? 1 : 0;
        }
        return
            strengthWeight[account].mul(rewardApplicable)
    ...
    /**
     * @dev Init supported Nft tokenId
     */
    function initNftToken(uint256 _tokenId, uint256 _maxAmount, uint256 _strengthWeight, uint256 _strengthDecimals) public onlyOwner {
        require(!nftTokenMap[_tokenId].hasValue, 'NFT token is already defined');
        nftTokens.push(_tokenId);
        nftTokenMap[_tokenId] = NftToken({ hasValue: true, active: true, maxAmount: _maxAmount, strengthWeight: _strengthWeight, strengthDecimals: _strengthDecimals });
    }
    
    /**
     * @dev Init supported Nft tokenId with batch
     */
    function initNftTokenBatch(uint256[] calldata _tokenIds, uint256[] calldata _maxAmounts, uint256[] calldata _strengthWeights, uint256[] calldata _strengthDecimals) public onlyOwner {
        require(_tokenIds.length == _maxAmounts.length && _tokenIds.length == _strengthWeights.length && _tokenIds.length == _strengthDecimals.length, 'Invalid params length');
        
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            initNftToken(_tokenIds[i], _maxAmounts[i], _strengthWeights[i], _strengthDecimals[i]);
        }
    }

    /**
     * @dev Update supported Nft tokenId
     */
    function updateNftToken(uint256 _tokenId, uint256 _maxAmount, uint256 _strengthWeight, uint256 _strengthDecimals) public onlyOwner {
        require(nftTokenMap[_tokenId].hasValue, 'NFT token is not defined');
        nftTokenMap[_tokenId].maxAmount = _maxAmount;
        nftTokenMap[_tokenId].strengthWeight = _strengthWeight;
        nftTokenMap[_tokenId].strengthDecimals = _strengthDecimals;
    }

    /**
     * @dev Activates supported Nft tokenId
     */
    function activateNftToken(uint256 _tokenId) public onlyOwner {
        nftTokenMap[_tokenId].active = true;
    }

    /**
     * @dev Deactivates supported Nft tokenId
     */
    function deactivateNftToken(uint256 _tokenId) public onlyOwner {
        nftTokenMap[_tokenId].active = false;
    }

    /**
     * @dev Sets main nft stake requirement
     */
    function setNftRequired(bool _nftRequired) public onlyOwner {
       nftRequired = _nftRequired;
    }

    /**
     * @dev Sets withdraw reward fee
     */
    ...
    /**
     * @dev Change contract addresses of ERC-20, ERC-1155 tokens
     */
    function changeAddresses(address _NDR, address _NFT, address _NFTSTAKING) public onlyOwner {
        NDR = IERC20(_NDR);
        NFT = IERC1155(_NFT);
        NFTStaking = INFTStaking(_NFTSTAKING);
    }
    ...
    function stakeInternal(uint256 tokenId, uint256 amount) internal {
        require(nftTokenMap[tokenId].active, "wrong id");
        require(nftTokenMap[tokenId].balances[msg.sender].add(amount) <= nftTokenMap[tokenId].maxAmount, "NFT max");
        
        uint256 strength = nftTokenMap[tokenId].strengthWeight.div(nftTokenMap[tokenId].strengthDecimals).mul(amount);
    ...
    /**
     * @dev Withdraw all ERC-1155 tokens
     */
    function withdraw() public updateReward(msg.sender) {
        uint8 length = 0;
        for (uint8 i = 0; i < nftTokens.length; i++) {
            uint256 balance = nftTokenMap[nftTokens[i]].balances[msg.sender];
            if(balance > 0) {
                length++;
            }
        }
    ...
    /**
     * @dev Unstake all cards and get all rewards
     */
        ...
        updateReward(address(0))
    {
        ...
        uint256 balance = NDR.balanceOf(address(this));
        require(
