...
abstract contract ERC721URIStorage is ERC721 {
    ...
    function makeArt(uint256 numberOs) public view virtual returns (string memory){
        ...
        uint rowCounter = 0;
        rowCounter = ((score + 8) / (16 * (1 + yearMult)));
        string memory finalString;
        string memory currentLetter;

        for (uint256 i = 0; i <= score + 7; i++) {
            if (i == 0){
                currentLetter = 'C';
            }
            else if (i == 1){
            ...
            OGArray[(i / (16 * (1 + yearMult)))] = concatenate(OGArray[(i / (16 * (1 + yearMult)))], currentLetter);
        }
...
constructor(address initialRoyaltiesReceiver) ERC721("Crypto.OG", "COG") {
    ...
    function mint(uint256 noYears) external payable {
        ...
        require(msg.value >= 20000000000000000 wei, "You must pay at least 0.02 Eth per art");
