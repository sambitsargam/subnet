...
contract ReservaX is ERC1155Supply, Ownable, ReentrancyGuard {
    ...
    external payable whenNotPaused onlyValidAccess(_v, _r, _s) {
        require(block.timestamp >= preSaleWindowOpens && block.timestamp <= preSaleWindowCloses, "Early access: window closed.");
        require(purchaseTxsEarly[msg.sender] + amount <= maxTxEarly , "Max mint amount exceeded, only 10 mint at early purchase.");
    ...
    function publicPurchase( uint256 amount, uint256 index) external payable whenNotPaused {
        require(block.timestamp >= publicWindowOpens && block.timestamp <= publicWindowCloses, "Public access: window closed.");
        require(purchaseTxs[msg.sender] + amount <= maxTxPublic , "Max mint amount exceeded, only 10 mint at public purchase.");
    ...
    function _purchase(uint256 amount, uint256 index) private {
        ...
        require(msg.value >= amount * mintPrice[level], "Purchase: Incorrect payment, check your wallet to see if you have the correct funds.");
        require(totalSupply(index) + amount <= maxSupplyEach[index], "Max supply reached for the art selected, please choose another.");
    ...
    function newCost(uint16[] calldata _newCost) external onlyOwner {
        mintPrice = _newCost;
    }
