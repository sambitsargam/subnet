...
contract HOHOHOTOKEN is Context, IERC20, Ownable {
    ...
    string private _name = "HO HO HO";
    string private _symbol = "HHHO";
    uint8 private _decimals = 9;

    uint256 public _taxBuyFee = 1;
    uint256 public _taxSellFee = 1;
    ...
    uint256 public _marketingBuyFee = 96;
    uint256 public _marketingSellFee = 6;
    uint256 public _marketingTransferFee = 6;

    uint256 public _charityBuyFee = 2;
    uint256 public _charitySellFee = 2;
    uint256 public _charityTransferFee = 2;

    uint256 public _liquidityBuyFee = 1;
    uint256 public _liquiditySellFee = 1;
    ...
    uint256 public _maxWalletHolding = 1 * 10**6 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 300 * 10**3 * 10**9;
    ...
    function _transfer(
        ...
        if (!isTransferBuy && !isTransferSell) {
          ...
          transferType = 1;
          if (_firstBuyTime[from] != 1 && (_firstBuyTime[from] + (24 hours) > block.timestamp) ) {
            //doubling sell tax when user is flipping within 24 hrs
            _marketingSellFee = _marketingSellFee.mul(1);
            _liquiditySellFee = _liquiditySellFee.mul(1);
            _burnSellFee = _burnSellFee.mul(1);
            _taxSellFee = _taxSellFee.mul(1);
            _charitySellFee = _charitySellFee.mul(1);
            _operationsFee = _operationsFee.mul(1);
            _liquidityFee = _liquidityFee.mul(1);
            _taxFee = _taxFee.mul(1);
            _burnFee = _burnFee.mul(1);
          }
        }
