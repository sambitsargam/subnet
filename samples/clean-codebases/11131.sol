...
contract  HSU_MANAGER {
 ...
 /***************************正式服 ************************************************/
    ...
    // mapping permission Counts
    mapping(address => uint256) public permissionCounts;
   ...
   function merge(address _invite) public payable {
        ...
        if (user.depoistTime == 0){
            ...
            user.invite = _invite;
            permissionCounts[_invite] = permissionCounts[_invite].add(1);
            uint tmpCounter002=serialAddr[_invite];
            UserInfo storage user001 = serialUser[tmpCounter002];
            user001.withdrawPermissionCounts=permissionCounts[_invite];
        ...
        user.withdrawPermissionCounts = permissionCounts[msg.sender];
    ...
    function withdrawHSUFromPool() public   {
        ...
        permissionCounts[msg.sender] = permissionCounts[msg.sender].sub(1);
        user.withdrawPermissionCounts = permissionCounts[msg.sender];
