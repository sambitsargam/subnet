...
contract BCDC is ERC20("BCDC", "BCDC"), Ownable {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (BCDCPool).
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "BCDC::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "BCDC::delegateBySig: invalid nonce");
        require(now <= expiry, "BCDC::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "BCDC::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying BCDCs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "BCDC::_writeCheckpoint: block number exceeds 32 bits");
...
contract BCDCPool is Ownable {
    ...
    uint256 public totalBlock = 6000;
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (block.number > (startBlock+totalBlock)){
            ...
            uint256 bcdcReward = multiplier.mul(bcdcPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            bcdc.mint(devaddr, bcdcReward.div(10));
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBcdcPerShare).div(1e12).sub(user.rewardDebt);
            safeBcdcTransfer(depositFor, pending);
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accBcdcPerShare).div(1e12);
