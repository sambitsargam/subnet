...
contract Marketplace is ReentrancyGuard, Pausable, Ownable {
    ...
    uint256 public basisPoints = 100;
    uint256 public maxTokens = 7500;
    ...
    function removeFromSale(uint index) public nonReentrant() {
        ...
        if (nft.ownerOf(index) != msg.sender) revert('you are not the owner of this token');
    ...
    function offerForSale(uint index, uint minSalePriceInWei) public whenNotPaused nonReentrant()  {
        ...
        nft.approve(address(this), index);
    ...
    function buy(uint index) payable public whenNotPaused nonReentrant() {
        ...
        if (seller != nft.ownerOf(index)) revert('seller no longer owner');


        saleOffers[index] = Offer(false, index, msg.sender, 0, address(0x0));
        nft.safeTransferFrom(seller, msg.sender, index);

        updateWithdrawals(seller, msg.value);
    ...
    function enterBid(uint index) payable public whenNotPaused nonReentrant() {
        if (index >= maxTokens) revert('token index not valid');
        if (nft.ownerOf(index) == msg.sender) revert('you already own this');
    ...
    function acceptBid(uint index, uint minPrice) public whenNotPaused nonReentrant() {
        if (index >= maxTokens) revert('token index not valid');
        if (nft.ownerOf(index) != msg.sender) revert('you do not own this token');
        ...
        bids[index] = Bid(false, index, address(0x0), 0);
        nft.safeTransferFrom(msg.sender, bidder, index);

        updateWithdrawals(seller, amount);
    ...
    function updateWithdrawals(address seller, uint amount) private {
        uint256 txPlatformFee = amount.div(basisPoints).mul(platformFee);
        uint256 remainder = amount - txPlatformFee;
