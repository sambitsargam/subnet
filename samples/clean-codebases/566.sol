...
contract InkongnitoNodes is Ownable {
    ...
    IERC20 public inkongToken = IERC20(0xB64E31Dd2812bDDdd7aC590E9d9Ff5bf6073348f);  //token contract
    ...
    constructor() {
        ...
        claimFee = 5;
        lpAddress = address(0x1990fA914D5eB52B9136E33ed70ACfcc79766cC0);  // deployer address
    }
    ...
    function createNode(string memory name, string memory desc, uint256 amount) external {
        ...
        require(inkongToken.balanceOf(msg.sender) > totalCost, "Insufficient Funds in wallet");

        uint256 lpFee = totalCost.mul(10).div(100);

        inkongToken.transferFrom(msg.sender, address(this), totalCost.sub(lpFee));
        inkongToken.transferFrom(msg.sender, lpAddress, lpFee);
    }
    ...
    function claimEarnings(uint256 nodeId) external nodeExists(msg.sender, nodeId) {
        ...
        inkongToken.transfer(msg.sender, reward);
        inkongToken.transfer(lpAddress, feeAmt);
    ...
    function claimAllEarnings() external {
        ...
        inkongToken.transfer(msg.sender, totalClaim);
        inkongToken.transfer(lpAddress, feeAmt);
    }
