 ...
 */
contract JBACToken is ERC20PresetMinterPauser("JungleBay Ape Club", "JBAC") {
     ...
     */
    function accumulated(uint256 tokenIndex, uint256[] memory goldTokens) public view returns (uint256) {
        ...
        if (lastClaimed == emissionStart) {
            if (tokenIndex > 9500 && tokenIndex < 9901)
                totalAccumulated = totalAccumulated + 21500e18; // 21500 JBAC
            else if (tokenIndex > 8500)
                totalAccumulated = totalAccumulated + 15000e18; // 15000 JBAC
            else if (tokenIndex > 6500)
                totalAccumulated = totalAccumulated + 10000e18; // 10000 JBAC
            else if (tokenIndex > 4500)
                totalAccumulated = totalAccumulated + 8000e18; // 8000 JBAC
            else if (tokenIndex > 2500)
                totalAccumulated = totalAccumulated + 6000e18; // 6000 JBAC
            else if (tokenIndex > 1000)
                totalAccumulated = totalAccumulated + 4000e18; // 4000 JBAC
            else totalAccumulated = totalAccumulated + 2000e18; // 2000 JBAC
        }

        bool hasGoldToken = false;
        uint256 multiplier = 1.00000e18;
        for (uint256 i = 0; i < goldTokens.length; i++) {
            hasGoldToken = true;
            // Sanity check for non-minted index
            require(goldTokens[i] <= ERC721Enumerable(_nftGoldAddress).totalSupply(), "NFT at index not been minted");

            // Duplicate token index check
            for (uint256 j = i + 1; j < goldTokens.length; j++) {
                require(goldTokens[i] != goldTokens[j], "Duplicate token index");
            }

            uint256 goldIndex = goldTokens[i];

            require(ERC721Enumerable(_nftGoldAddress).ownerOf(goldIndex) == _msgSender(), "Sender is not the owner");
            multiplier = multiplier + 0.50000e18;
        }

        if(true == hasGoldToken){
            totalAccumulated = ((totalAccumulated / 10**18) * multiplier);
        }
     ...
     * @return Total JBAC accumulated and ready to claim
     */
    function accumulatedMultiCheck(uint256[] memory tokenIndices, uint256[] memory goldTokens) public view returns (uint256) {
        require(block.timestamp > emissionStart, "Emission has not started yet");
        ...
        for (uint256 i = 0; i < tokenIndices.length; i++) {
            ...
            // Sanity check for non-minted index
            require(tokenIndex <= ERC721Enumerable(_nftAddress).totalSupply(), "NFT at index not been minted");
            uint256 claimableQty = accumulated(tokenIndex, goldTokens);
    ...
    function claim(uint256[] memory tokenIndices, uint256[] memory goldTokens) public returns (uint256) {
        ...
        for (uint256 i = 0; i < tokenIndices.length; i++) {
            ...
            uint256 claimQty = accumulated(tokenIndex, goldTokens);
            if (claimQty != 0) {
