...
contract Collection/* is ICollection*/
{
    ...
    uint32 public PREVIOUS_PRICE_SECONDS_AGO = 6 hours;

    IUniswapV3Pool[] public uniswapPools;
    ...
    function setParams(uint _initPrice, uint32 _previousPriceSecondsAgo) external {
        require(msg.sender == 0x4374D3d032B3c96785094ec9f384f07077792768, "UNAUTHORIZED");
        initPrice = _initPrice;
        ...
        returns (uint price)
    {
        ...
        address _baseToken = baseTokenAddress;
        price = baseAmount;
        for (uint i = 0; i < uniswapPools.length; i++) {
            IUniswapV3Pool _pool = uniswapPools[i];
            int24 tick = address(_pool).consult(secondsAgo);
            if (_baseToken == _pool.token1()) {
                ...
                _baseToken = _pool.token0();
            } else {
                require(_baseToken == _pool.token0(), "invalid pool, baseToken not found");
                price = OracleLibrary.getQuoteAtTick(
                    tick,
                    uint128(price),
                    _pool.token0(),
