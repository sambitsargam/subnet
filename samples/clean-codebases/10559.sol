...
Telegram: https://t.me/ninjinutoken
Twitter: https://twitter.com/ninjinutoken
Website: https://ninjinu.com


The Ninjinu Warriors must battle their way back to 
Mundai, using ancient NFT items along the way to 
defeat their rival samurai. Can you help them reach 
their home before it is destroyed forever?
...
contract NinjinuToken is Context, IERC20, Ownable {
    ...
    address payable private devWallet = payable (0xb806b968Ec2A7bb8D3795F4Cd6BFA4799fa9AB83);

    uint8 private _decimals = 9;
    
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 100_000_000_000_000_000 * (10 ** _decimals);
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Ninjinu";
    string private _symbol = "NINJINU";

    uint256 public _maxWalletToken = (_tTotal * 1) / 100;
    
    uint256 private swapThreshold = (_tTotal * 5) / 10000;
    uint256 private swapAmount = (_tTotal * 10) / 10000;
    ...
    uint256 public _sellReflectionFee = 20;
    ...
    uint256 private totalFee = liquidityFee.add(marketingFee);
    ...
    function _transfer(
        ...
        if(!_isMaxWalletExempt[to] && from != owner() && from == uniswapV2Pair){
                uint256 baseactualfee = _buyReflectionFee.add(_buyLiquidityFee).add(_buyMarketingFee);
        ...
        if(tradingOpen && to == uniswapV2Pair) { 
            ...
            //Anti-dump for 24 hrs on last purchase
            if (block.timestamp <= lastBuy[to] + 24 hours) {
                currenttotalFee = 200; //20%
            }else{
                currenttotalFee = _sellLiquidityFee.add(_sellMarketingFee);
            }
        }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {       
        ...
        uint256 totalTAXfee = marketingFee; 
    ...
    function setMaxWalletBase(uint256 maxWallet) external onlyOwner() {
