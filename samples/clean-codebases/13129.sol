...
contract FundamentaWrappedToken is SecureContract, ERC20
{
    ...
    event FeeCounterReset();

    using SafeERC20 for IERC20;

    address private _backingToken;
        ...
        ERC20(name, ticker)
    {
        ...
        _backingToken = backingTokenContract;
    ...
    function setBackingToken(address backingTokenContract) public isAdmin
    {
    ...
    function queryBackingToken() public view returns (address) { return _backingToken; }
    ...
    {
        require(_backingToken != address(0), "Wrapped Token: Backing token is not configured");
        ...
        IERC20 backingToken = IERC20(_backingToken);

        uint256 fee = calculateWrapFee(amount);
        require(backingToken.balanceOf(msg.sender) >= amount, "Wrapped Token: Insufficient backing token balance");
    ...
    function unwrap(uint256 amount) public pause
    {
        ...
        require(backingToken.balanceOf(address(this)) >= (amount - fee),
            "Wrapped Token: Insufficient backing token available");

        _burn(msg.sender, amount);
        backingToken.safeTransfer(msg.sender, amount - fee);
        _accumulatedFee += fee;
    ...
    function crossChainWrap(address user, uint256 amount) public isBridge pause returns (uint256)
    {
        ...
        uint256 fee = calculateWrapFee(amount);
        require(backingToken.balanceOf(user) >= amount, "Wrapped Token: Insufficient backing token balance");
        
        backingToken.safeTransferFrom(user, address(this), amount);
        _accumulatedFee += fee;
    ...
    function crossChainUnwrap(address user, uint256 amount) public isBridge pause
    {
        ...
        backingToken.safeTransfer(user, amount - fee);
        _accumulatedFee += fee;
    ...
    function withdrawAccumulatedFee(address to, uint256 amount) public isOwner
    {
        require(_accumulatedFee > 0, "Wrapped Token: Accumulated fee = 0");
        require (amount <= _accumulatedFee, "Wrapped Token: Attempted fee withdrawal too high");

        uint256 withdrawAmount = amount;

        if (withdrawAmount == 0)
            withdrawAmount = _accumulatedFee;

        IERC20 backingToken = IERC20(_backingToken);

        require(backingToken.balanceOf(address(this)) >= withdrawAmount, "Wrapped Token: Insufficient backing token available");

        backingToken.safeTransfer(to, withdrawAmount);
        _accumulatedFee -= withdrawAmount;

        emit FeeWithdraw(to, withdrawAmount);
    }

    function resetFeeCounter() public isOwner
    {
        require(_accumulatedFee > 0, "Wrapped Token: Accumulated fee = 0");
        _accumulatedFee = 0;
        emit FeeCounterReset();
    }
}