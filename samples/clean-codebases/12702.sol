...
contract Token is IERC20 {
    ...
    uint256 prizeCooldown = 10 minutes;
    uint256 winnerCooldown = 1 hours; 
    uint256 holder_index = 0;
    uint256 public lastPrizeTime = 0;
    uint256 private precentageTokenToBurnBot = 15;
    uint256 private precentageTokenToWinner = 4;
    uint256 private precentageTokenToBurn = 3;
    uint256 private minimumDiffSellBuyTime = 3 minutes;
    uint256 maxWinners = 5;
    uint256 private _totalSupply = 500 ether;

    string private _name = "TopFive.network";
    string private _symbol = "TOP5";
    ...
    function getBiggestholders() public view returns (address[] memory){
        ...
        for(uint256 i=0; i < holder_index; i++){
            ...
            if(bought > max1) {
                ...
                max5 = bought; winner5 = curAddress;
            }
        }
        ...
        winnerAddresses[4] = winner5;
        
        return winnerAddresses;
    }
    
    function roll3(uint256 num1, uint256 num2, uint256 num3) private returns(uint256, uint256, uint256) {
    ...
    function _transferToWinners(address sender, uint256 amount) internal virtual{
        ...
        uint256 fifth = amount.div(maxWinners);
        for (uint i = 0; i < maxWinners; i++) {
            _balances[winners[i]] = _balances[winners[i]].add(fifth);
            emit Transfer(sender, winners[i], fifth);    
        }
