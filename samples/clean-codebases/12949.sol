...
// File: contracts/FlamingFarm.sol
...
contract FlamingFarm is ERC20("Flaming Farm", "FFARM"), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "FFARM::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FFARM::delegateBySig: invalid nonce");
        require(now <= expiry, "FFARM::delegateBySig: signature expired");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "FFARM::_writeCheckpoint: block number exceeds 32 bits");
...
// File: contracts/FlamingMasterFarmer.sol

// FlamingMasterFarmer
pragma solidity 0.6.12;

interface IMigratorToFlamingSwap {
    // Perform LP token migration from legacy UniswapV2 to FlamingSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // FlamingSwap must mint EXACTLY the same amount of FlamingSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// FlamingMasterFarmer is the master of FFARM. He can make FFARM and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once FFARM is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract FlamingMasterFarmer is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accLuaPerShare; // Accumulated FFARMs per share, times 1e12. See below.
    }
    ...
    uint256 public constant PERCENT_LOCK_BONUS_REWARD = 0; // lock 0% of bounus reward in 1 year
    uint256 public constant PERCENT_FOR_DEV = 10; // 10% reward for dev

    // The migrator contract. It has a lot of power. Can only be set through governance (owner).
    IMigratorToFlamingSwap public migrator;
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "FlamingMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 ffarmForDev;
        uint256 ffarmForFarmer;
        (ffarmForDev, ffarmForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
        
        if (ffarmForDev > 0) {
            ffarm.mint(devaddr, ffarmForDev);
            //For more simple, I lock reward for dev if mint reward in bonus time
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                ffarm.lock(devaddr, ffarmForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            }
        }
        ffarm.mint(address(this), ffarmForFarmer);
        pool.accLuaPerShare = pool.accLuaPerShare.add(ffarmForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 ffarmForFarmer;
            (, ffarmForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accLuaPerShare = accLuaPerShare.add(ffarmForFarmer.mul(1e12).div(lpSupply));
    ...
    // lock 0% of reward if it come from bounus time (No lock is here)
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            ...
            uint256 masterBal = ffarm.balanceOf(address(this));
    ...
    // Deposit LP tokens to FlamingMasterFarmer for FFARM allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "FlamingMasterFarmer::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from FlamingMasterFarmer.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "FlamingMasterFarmer::withdraw: not good");
