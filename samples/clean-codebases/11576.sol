...
contract BUTTERface is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 1_250_000 * 10**9;
    uint256 public maxBuyLimit = 2_500_000 * 10**9;
    uint256 public maxSellLimit = 2_500_000 * 10**9;
    ...
    address public devWallet = 0xcb01c5FB4A779FBe22fDCE7ab6c1B97fC4e6fFAE;
    address public Charity = 0x74B198018A752c8E9529C77aEE60376DC4444A97;

    string private constant _name = "BUTTERface";
    ...
    Taxes public taxes = Taxes(0, 5, 4, 2, 0);
    Taxes public sellTaxes = Taxes(0, 7, 4, 6, 0);
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _dev, uint256 _Charity) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_dev,_Charity);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _dev, uint256 _Charity) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_dev,_Charity);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rDev, to_return.rCharity) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tDev-s.tCharity;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rDev-rCharity;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rDev,uint256 rCharity) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tDev+ s.tCharity);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.dev + temp.Charity) * 2;
        ...
        if(CharityAmt > 0){
            payable(Charity).sendValue(CharityAmt);
        }
