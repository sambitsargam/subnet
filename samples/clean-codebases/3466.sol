...
contract VikingsValhalla is Context, IERC20, Ownable, Pausable {
    ...
    string private _name = "Vikings: Valhalla";
    string private _symbol = "VKG";
    ...
    address payable public marketingWallet = payable(0xB002e7Ec2Fe3e798A82751d52F912948E68909d8);
 
    uint256 public _treasuryFee = 1;
    uint256 private _previousTreasuryFee = _treasuryFee;
    address payable public treasuryWallet = payable(0xeE6EDff8eCaD7BCd1433c3cE8409daFA44A18074);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee==0 && _treasuryFee==0) return;
    ...
    function _transfer(
        ...
        if (
            ...
            //add liquidity and swap for marketing and treasury wallet
            swapAndLiquify(contractTokenBalance);
        }
       
        //transfer amount, it will take tax, liquidity, marketing and treasury fee
        _tokenTransfer(from,to,amount);
    }
 
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(_marketingFee.add(_liquidityFee).add(_treasuryFee));
        uint256 tokensForMarketing = contractTokenBalance.mul(_marketingFee).div(_marketingFee.add(_liquidityFee).add(_treasuryFee));
        uint256 tokensForTreasury = contractTokenBalance.sub(tokensForLiquidity.add(tokensForMarketing));
        ...
        // swap and send eth to marketing wallet
        swapTokensForEth(tokensForMarketing, marketingWallet);
 
        // swap and send eth to Treasury wallet
    ...
    function setFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 treasuryFee) external onlyOwner() {
        _taxFee = taxFee;
